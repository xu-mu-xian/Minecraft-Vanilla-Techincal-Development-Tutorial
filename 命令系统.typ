...
一般而言，原点参数需要配合距离、体积、排序或数量参数使用，*单独定义原点参数的值是无效的*。
==== 距离参数
...
#codebox("[team=!]")
==== 记分板变量参数
变量参数的作用为：当实体在某一个或多个变量上的分数同时满足定义的分数范围时，该实体就会被选择。变量参数支持同时定义多个变量的选择范围，语法格式较一般的目标选择器参数有所不同，即在参数值的位置上加花括号，在花括号中定义各变量的分数范围，变量之间用逗号隔开：
