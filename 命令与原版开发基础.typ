...
#example(
  [
    有资源包纹理文件路径为 `minecraft > textures > block > command_block_front.png`，试用命名空间ID指定之。
  ],
  [
...
一个 #icon("folder") `data` 文件夹中可以存在多个不同的命名空间，而命名空间 `minecraft` 下的内容会覆盖原版游戏内容。一旦不同的数据包使用的命名空间有重复，则相同路径、相同名称的文件会发生冲突。因此在创建数据包的过程中，需要尽量使用标识鲜明的命名空间以防止和其他数据包发生冲突。
...
#codebox("#<命名空间>:<标签>")
若命名空间不写，则默认使用 `minecraft` 内的数据包标签。`<注册表>` 层级下可以添加一定的路径。例如，#icon("json") `data > <命名空间> > tags > <注册名> > <路径> > <标签>.json` 的引用格式为
#codebox("#<命名空间>:<路径>/<标签>")
...
=== 朝向与局部坐标的关系 \* <subsec:rotate_and_local_coordinates>
...
#example(
  [“色盲派对”是常见的服务器小游戏：在一块平台上会有不同颜色的方块，每轮都会给出一种颜色的方块，倒计时结束时，不是该颜色的方块就会消失，届时位于上面的玩家就会掉入虚空。假如游戏平台位于$(0,0,0)$和$(31,0,31)$之间，平台上会出现棕、红、橙、黄、黄绿、绿、青、淡蓝、蓝、紫、品红、粉红这几种颜色的混凝土。假如一轮游戏中不是红色混凝土的方块都会消失，尝试实现这个效果。],
  [
...
#tree(
  ...
  (1, [#icon("nbt-bool")#icon("json-bool") *obfuscated*: 是否将文字#proper-noun(display: "模糊化（Obfuscated）", "mo2 hu2 hua4")处理，使文本渲染为动态的随机字符（乱码）。]),
...
=== 实体共通标签
与方块实体类似，所有实体的根标签下一定有一些共同存在的子标签，这些标签不会因为实体种类的变化而有所改变，于是称这类标签为#proper-noun(display: "实体共通标签（Tags common to all entities）", "shi2 ti3 gong4 tong1 biao1 qian1")。下面列举了所有的实体共有的字段，注意，不是所有的实体共通标签都能被命令 `/data` 所修改。<data:tags_common_to_all_entities>
#tree(
  (0, [#icon("nbt-compound") 根标签]),
...
对于@equ:scale_matrix 而言，特别地、若$m$、$n$、$p$三者至少有一个为负数，则变换有镜像成分。负缩放因子使坐标系在对应轴上反转，表面法线方向改变，从而造成内凹渲染。
#figure(
  caption: "镜像造成的内凹渲染",
  image("图片/镜像造成的内凹渲染.png", width: 18em)
)
====== 剪切
...
通常而言，物品拥有堆叠数量、命名空间ID和附加信息这些属性。特别地，一些物品被存储在实体或方块实体的特定槽位中，则它们使用带槽位的物品格式。如果一个物品为某个属性类的数据或配置项，如实体物品中的物品，则使用不带槽位的物品格式。<data:item_data>
#tree(
  (0, [#icon("nbt-compound") 根标签]),
...
#param-desc(
  ...
  [`[<maxCount>]`（整型 `brigadier:integer`）], [可选，用于指定被清除物品可被清除的最大数量，必须为整数。若该参数小于被清除物品的数量，则被清除物品被清除的数量就为该参数指定的数量；若该参数大于或等于被清除物品的数量，则被清除物品会被全部清除；若该参数为0，则*没有任何物品会被清除，并返回指定物品的数量*。例如，有命令 `clear @s stone 10`，这时如果命令执行者自身有20个石头，则该命令会清除10个石头；如果命令执行者自身有5个石头，则该命令会清除5个石头。]
)
==== 命令 `/give`
...
在 `/give`、`/item` 等命令的格式中为了确认一个物品及其使用的组件，使用参数类型 `minecraft:item_stack`，其语法为
...
#tree(
  ...
  (4, [#icon("nbt-string")#icon("nbt-list") *items*: 匹配的物品，可以是 #icon("nbt-string") 命名空间ID或物品数据包标签。也可以使用 #icon("nbt-list") 列表形式以尝试匹配列表中任意物品。]),
...
+ 判断下列与属性有关的说法是否正确。
  + 在任何情况下，属性的值$eq.not$属性基值$eq.not$属性修饰符的值。
...
#general-table(
  ...
  columns: (auto, auto, auto),
  header: ([显示位置], [简介], [图例]),
  [`sidebar`（侧边栏）], [在屏幕的右侧显示。显示名称会显示在最上方。只会显示有分数的分数持有者的分数，若为玩家则显示玩家名，若为其他实体则显示UUID，带 `#` 号假名的分数不会出现在侧边栏，但 `$` 开头的名字会正常显示。如果玩家不在线，其分数依然会被显示。], [#image("图片/显示位置侧边栏.png", width: 10em)],
...
#param-desc(
  [`<target>`（分数持有者 `minecraft:score_holder`）], [需要获取分数的分数持有者，可以是玩家名、UUID或目标选择器，只能指定1个分数持有者。]
)
===== `list`
该子命令用于返回指定分数持有者在所有追踪该分数持有者的记分项上的分数，语法为
#codebox("scoreboard players list [<target>]")
#param-desc(
  [`[<target>]`（分数持有者 `minecraft:score_holder`）], [可选，若不填写该参数，则返回所有正在被记分项追踪的分数持有者。若指定此参数，则只能指定1个分数持有者。]
)
===== `operation`
...
=== 秒表条件：stopwatch
判断秒表的时间是否在指定范围内，语法为；
#codebox("(if|unless) stopwatch <id> <range> -> execute")
#param-desc(
  [`<id>`（命名空间ID `minecraft:resource_location`）], [指定的秒表，必须为已存在秒表的命名空间ID。],
  [`<range>`（浮点数范围 `minecraft:float_range`）], [指定的时间范围，必须为由单精度浮点数构成的范围，有精确值（如 `0.1`）、单侧限制（如 `0.1..` 或 `..0.1`）和双侧限制（如 `-0.1..0.1`）几种。表示范围时限制值也包含在内，即在数学上是闭区间。例如，`0.1..` 表示大于或等于0.1。]
)
#example(
  [判断秒表 `leather_sword` 的时间是否在700秒及以内。],
  [
    命令为
    #codebox("execute if stopwatch leather_sword ..700")
  ]
)
=== 应用实例
下面举条件子命令应用的若干实例。
#example(
  [坐标$(0,70,0)$处有一个箱子，当箱子中同时存储了TNT和打火石时，在箱子的位置生成一个立刻爆炸的TNT。],
  [
    箱子拥有方块实体，因此可以用 `if` 子命令对容器内的物品进行判断。连续使用两次 `if` 子命令，分别判断TNT和打火石即可，命令为
    #codebox("execute if items block 0 70 0 container.* minecraft:tnt if items block 0 70 0 container.* minecraft:flint_and_steel run summon tnt 0 70 0")
    #h(-2em)生成TNT实体的子命令内没有添加任何NBT，这是因为不指定引信时间的TNT实体会在生成的瞬间爆炸。
  ]
)
#example(
...
#example(
  ...
  [
    遇到有关胜负的情境时，通常的思路是将胜负转化为分数，然后对分数进行判断。根据题意，不妨先建立两个队伍：蓝队和绿队。

    由于存在 `teamkill` 或 `killedByTeam` 这两种准则，可能的办法是分别创建一个准则为 `teamkill` 的记分项以分别统计杀死蓝队和绿队玩家的数量。使用了这两种准则的记分项能够自动统计杀死指定队伍成员的行为，但同时也带来一个问题：该准则针对分数做出的修改都是以单个分数持有者为基准的。假设蓝队成员A消灭了对方队伍中2名成员，B消灭了对方队伍中1名成员，根据游戏规则，这时蓝队已经获胜。有一种设想是将所有属于蓝队成员的分数相加，不过必须得考虑以下几个问题：
    + 何时将蓝队成员的分数相加？如果使分数相加的命令高频执行，则每游戏刻都会进行加法运算，最后得到的结果是一个巨大的数字，显然这种方法不可取。若在每次记分板上某分数持有者的分数更改时进行加法操作，则需要判断分数的改变，显然这种子命令是不存在的。
    + 若对方队伍中的成员阵亡原因不是本队的成员怎么办？在服务器小游戏的一箭必杀中，玩家是处于平台上的，掉下平台也是有可能的。此时仍会被判定为玩家阵亡。如此看来，玩家的阵亡不一定是由其他队伍的玩家造成的，因此要避免使用像 `teamkill` 或 `killedByTeam` 这样的准则，最好改为纯粹的死亡计数。
    + 阵亡的玩家不可再进行本轮游戏，于是需要让所有阵亡的玩家保持旁观模式。
    \
    
    所以需要被 `#minecraft:load` 调用的函数为：
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > load.mcfunction",
      "team add blue {text:\"Blue\",color:\"blue\"}
team add green {text:\"Green\",color:\"green\"}
scoreboard objectives add deaths deathCount
scoreboard objectives add rounds dummy"
    )
    需要被 `#minecraft:tick` 调用的函数为：
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > tick.mcfunction",
      "#旁观模式
execute as @a if data entity @s {Health:0} run gamemode spectator @s

#检测玩家死亡
execute as @a[team=blue] if score @s deaths matches 1 run function dodgebolt:blue_team/death
execute as @a[team=green] if score @s deaths matches 1 run function dodgebolt:green_team/death

#检测队伍在一轮游戏中获胜
execute if score #blue deaths matches 3.. run function dodgebolt:blue_team/round_win
execute if score #green deaths matches 3.. run function dodgebolt:green_team/round_win

#检测队伍最终胜利
execute if score #blue rounds matches 3.. run function dodgebolt:blue_team/win
execute if score #green rounds matches 3.. run function dodgebolt:green_team/win"
    )
    其中两个队伍内有玩家死亡时需要运行的函数为：
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > blue_team > death.mcfunction",
      "scoreboard players reset @s deaths
scoreboard players add #blue deaths 1"
    )
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > green_team > death.mcfunction",
      "scoreboard players reset @s deaths
scoreboard players add #green deaths 1"
    )
    如果一个队伍在一轮游戏中获胜，则运行以下的函数：
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > blue_team > round_win.mcfunction",
      "scoreboard players add #blue rounds 1
scoreboard players reset #blue deaths
scoreboard players reset #green deaths
gamemode adventure @a[gamemode=spectator]
scoreboard players reset @a deaths"
    )
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > green_team > round_win.mcfunction",
...
scoreboard players reset @a deaths"
    ...
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > blue_team > win.mcfunction",
      "tellraw @a \"蓝队获胜\"
scoreboard players reset #blue rounds
scoreboard players reset #green rounds"
    )
    #codefile(
      lang: "mcfunction",
      title: "data > dodgebolt > function > green_team > win.mcfunction",
      "tellraw @a \"绿队获胜\"
scoreboard players reset #blue rounds
scoreboard players reset #green rounds"
    )
...
=== 存储格式
命令 `/execute` 只有一种存储子命令：`store`。其基本的语法可以表示为
#codebox("store (result|success) <存储途径> -> execute")
从语法中可以看到存储子命令提供的两种存储模式：`result` 和 `success`。顾名思义，`result` 模式用于存储命令执行的*结果*，结果的数值受语法影响不定，但*一定为整数，且会将浮点数向下取整*；`success` 模式用于存储命令执行的*成功次数*，一般而言，命令执行的成功次数不超过1次。同时，成功次数与结果有很强的关联性，若命令执行不成功，则成功次数为0，此时结果一定为0。

对于这种子命令执行后的实际效果，举一个例子以说明之：有命令
#codebox("execute store result score @s health run data get entity @s Health")
#h(-2em)这条命令的执行结果即为 `run` 子命令的执行结果，即获取命令执行者自身实体或玩家格式中 #icon("nbt-float") `Health` 标签的值。于是，命令执行结果即为 #icon("nbt-float") `Health` 标签的值。同时，`store` 子命令将这个值存入记分项 `[health]` 中，是为命令执行者在 `[health]` 上的分数。

如果有命令
#codebox("execute store success score @s health run data get entity @s Health")
#h(-2em)若 `run` 子命令中的 `/data` 命令执行成功，则命令执行的成功次数为1，将 `1` 存入记分项 `[health]`；若执行失败，则命令执行的成功次数为0，将 `0` 存入记分项 `[health]`。

`if` 和 `unless` 子命令的测试结果也能够由存储子命令处理。若条件子命令的测试通过且位于命令末尾，则执行成功次数为1。如果存在测试计数值，则结果即为计数值，如果不存在则结果为 `1`；若测试失败，则执行成功次数为0，此时无论测试计数值为何，结果一定为 `0`。现整理所有的单个条件在测试通过时的结果情况，列于下表：
#general-table(
  caption: "条件子命令在测试通过时的结果",
  colspan: 3,
  columns: (auto, auto, auto),
  header: ([条件], [#text(black)[`if`] 子命令的结果], [#text(black)[`unless`] 子命令的结果]),
  [biome], table.cell(colspan: 2)[1],
  [block], table.cell(colspan: 2)[1],
  [blocks], [区域内被探测方块的数量], [1],
  [data], [若指定了一个列表（或数组），则返回列表（或数组）中元素的个数，指定其他类型的标签，则结果为1], [1],
  [dimension], [匹配的维度数量], [1],
  [entity], [存在的符合要求的实体数量], [1],
  [function], table.cell(colspan: 2)[-（不能位于命令末尾）],
  [items], [匹配的物品总数], [1],
  [loaded], table.cell(colspan: 2)[1],
...
#general-table(
  ...
  columns: (auto, auto, auto),
  header: ([命令], [语法], [执行结果]),
  [`/advancement`], [], [给予或移除每个玩家进度的总数。],
  table.cell(rowspan: 4)[`/attribute`], [`... base get [<scale>]`], [属性基值与倍率的乘积。],
  [`... get [<scale>]`], [属性最终值与倍率的乘积。],
  [`... modifier value get <uuid> [<scale>]`], [属性修饰符的修饰量与倍率的乘积。],
  [其他], [`1`],
  table.cell(rowspan: 11)[`/bossbar`], [`... add ...`], table.cell(rowspan: 2)[添加或删除自定义Boss栏后剩余的自定义Boss栏总数，游戏自带的Boss栏不计（如末影龙和凋零）。],
  [`... remove ...`],
  [`... list ...`], [自定义Boss栏总数,游戏自带的Boss栏不计。],
  [`... set <ID> max ...`], table.cell(rowspan: 2)[Boss栏的最大值。],
  [`... get <ID> max`],
  [`... set <ID> value ...`], table.cell(rowspan: 2)[Boss栏的当前值。],
  [`... get <ID> value`],
  [`... set <ID> players ...`], table.cell(rowspan: 2)[可以看到该Boss栏的玩家数量。],
  [`... get <ID> players`],
  [`... get <ID> visible`], [若可见，则返回 `1`；不可见则返回 `0`。],
  [其他], [`0`],
  [`/clear`], table.cell(rowspan: 3)[], [被清除或检测到的物品总数。],
  [`/clone`], [目标区域内被更改的方块总数。],
  [`/damage`], [`1`],
  table.cell(rowspan: 8)[`/data`], [`... get ...`], [若不指定任何NBT路径，则返回值为 `1`；\ 若取得整型或浮点数标签，则返回向下取整后的值；\ 若取得数组或列表，则返回数组或列表中元素的数量；\ 若取得字符串标签，则返回字符串长度；\ 若取得复合标签，则返回其第一级子标签的数量。\ 若还指定了倍率，则需要返回值需要乘上倍率。],
  [`... merge ...`], [`1`],
  [`... modify ... append ...`], table.cell(rowspan: 3)[被加入新元素的列表数量。],
  [`... modify ... insert <index> ...`],
  [`... modify ... prepend ...`],
  [`... modify ... merge ...`], [成功更改的复合标签数量。],
  [`... modify ... set ...`], [成功更改的标签数量。],
  [`... remove ...`], [`1`],
  table.cell(rowspan: 4)[`/datapack`], [`... create ...`], [`1`],
  [`... disable ...`], table.cell(rowspan: 2)[命令执行后启用的数据包数量。],
  [`... enable ...`],
  [`... list ...`], [列出的数据包数量。],
  [`/defaultgamemode`], [], [在局域网内置服务器或 `force-gamemode` 被设为 `true` 的专用服务器中为默认游戏模式被更新的玩家数量，其他情况下结果为 `0`。],
  table.cell(rowspan: 2)[`/dialog`], [`... clear ...`], [清除的对话框数量。],
  [`... show ...`], [展示的对话框数量。],
  table.cell(rowspan: 2)[`/difficulty`], [无参数], [该游戏难度的ID：和平为 `0`，简单为 `1`，普通为 `2`，困难为 `3`。],
  [其他], [`0`],
  [`/effect`], [], [被施加或移除状态效果的实体总数。],
  [`/enchant`], [], [物品被附魔的玩家总数。],
  table.cell(rowspan: 3)[`/experience`\ `/xp`], [`... add ...`], table.cell(rowspan: 2)[被修改经验的目标总数。],
  [`... set ...`],
  [`... query ...`], [经验值或等级。],
  [`/fetchprofile`], [], [`1`],
  [`/fill`], [], [源区域内被更改的方块总数。],
  [`/fillbiome`], [], [被填充生物群系的区域的方块总数。],
  table.cell(rowspan: 5)[`/forceload`], [`... add ...`], [新增的强制加载区块总数。],
  [`... query`], [该维度中强制加载的区块总数。],
  [`... query <pos>`], [`1`],
  [`... remove <from> [<to>]`], [移除的强制加载区块总数。],
  [`... remove all`], [`0`],
  [`/function`], [], [所有被执行到的非Void函数的返回值之和。],
  [`/gamemode`], [], [游戏模式被更新的玩家数量。],
  [`/gamerule`], [], [更改后游戏规则的值。布尔值 `true` 为 `1`，`false` 为 `0`。],
  [`/help`], [], [列出的命令用法的数量。],
  table.cell(rowspan: 2)[`/item`], [`... block ...`], [`1`],
  [`... entity ...`], [被强制覆盖物品的实体总数。],
  [`/kill`], [], [被清除的实体总数。],
  [`/list`], [], [玩家总数。],
  [`/locate`], [], [执行者与指定的结构、兴趣点或生物群系之间的距离。],
  table.cell(rowspan: 5)[`/loot`], [`... give ...`], [所有玩家被给予的物品堆叠数量之和。],
  [`... insert ...`], [容器被插入的物品堆叠数量。],
  [`... replace block ...`], [容器被替换的物品堆叠数量。],
  [`... replace entity ...`], [实体被替换的物品堆叠数量。],
  [`... spawn ...`], [生成的实体物品堆叠的数量。],
  [`/me`], [], [`1`], 
  [`/msg`\ `/tell`\ `/w`], [], [指定的玩家数量。],
  [`/particle`], [], [能够看见该粒子效果的玩家总数。],
  [`/place`], [], [`1`],
  [`/playsound`], [], [能够听见该声音的玩家总数。],
  table.cell(rowspan: 2)[`/random`], [`... reset ...`], [重置规则的随机列个数。],
  [`... (value|roll) ...`], [抽取的随机值。],
  [`/recipe`], [], [为玩家解锁或锁定的配方总数。],
  [`/reload`], [], [`0`],
  table.cell(rowspan: 3)[`/return`], [`... fail`], [`0`],
  [`... run <command>`], [`<command>` 的返回值。],
  [`... <value>`], [`<value>`],
  [`/ride`], [], table.cell(rowspan: 3)[`1`],
  [`/rotate`], [],
  [`/say`], [],
  table.cell(rowspan: 2)[`/schedule`], [`... clear ...`], [被停止计划的函数数量。],
  [`... function ...`], [计划运行函数的游戏刻数。],
  table.cell(rowspan: 14)[`/scoreboard`], [`... objectives add ...`], table.cell(rowspan: 2)[命令执行后记分项的总数。],
  [`... objectives remove ...`],
  [`... objectives list ...`], [记分项的总数。],
  [`... objectives modify ...`], table.cell(rowspan: 2)[`0`],
  [`... objectives setdisplay ...`],
  [`... players add ...`], table.cell(rowspan: 3)[所有被修改分数的目标分数的总和。],
  [`... players operation ...`],
  [`... players remove ...`],
  [`... players enable ...`], [被启用触发器记分项的分数持有者总数。],
  [`... players get ...`], [指定分数持有者的分数。],
  [`... players list ...`], [追踪此分数持有者的记分项总数。],
  [`... players reset ...`], [被重置分数的分数持有者总数。],
  [`... players set ...`], [指定分数与分数持有者数量的积。],
  [其他], [`0`],
  [`/seed`], [], [转换后的32位世界种子。],
  [`/setblock`], [], table.cell(rowspan: 2)[`1`],
  [`/setworldspawn`], [],
  [`/spawnpoint`], [], [被设置重生点的玩家总数。],
  [`/spectate`], [], [`1`],
  table.cell(rowspan: 2)[`/spreadplayers`], [`... true ...`], [目标所在队伍的总数。],
  [`... false ...`], [目标的总数。],
  [`/stopsound`], [], [停止播放声音的玩家总数。],
  table.cell(rowspan: 2)[`/stopwatch`], [`... query ...`], [获取的秒表时间和 `<sacle>` 的乘积，若未指定 `<sacle>` 则获取原本的秒表时间，所得结果向下取整。],
  [其他], [`1`],
  [`/summon`], [], [`1`],
  [`/swing`], [], [被选中的挥动手臂的实体总数。],
  table.cell(rowspan: 3)[`/tag`], [`... add ...`], table.cell(rowspan: 2)[被添加或移除标签的实体总数。],
  [`... remove ...`],
  [`... list ...`], [指定实体的标签总数。],
  table.cell(rowspan: 11)[`/team`], [`... add ...`], table.cell(rowspan: 2)[添加或移除队伍后存在的队伍总数。],
  [`... remove ...`],
  [`... empty ...`], [清空队伍前队伍中的实体总数。],
  [`... join`], [`1`],
  [`... join <member>`], [加入队伍的实体总数。],
  [`... leave ...`], [离开队伍的实体总数。],
  [`... list`], [队伍总数。],
  [`... list <team>`], [指定队伍中实体总数。],
  [`... modify prefix ...`], table.cell(rowspan: 2)[`1`],
  [`... modify suffix ...`],
  [其他], [`0`],
  [`/teammsg`\ `/tm`], [], [队伍内的玩家数量。],
  [`/teleport`\ `/tp`], [], [传送的实体总数。],
  [`/tellraw`], [], [指定的玩家总数。],
  table.cell(rowspan: 3)[`/test`], [`... clear ...`], table.cell(rowspan: 2)[被清除或重置的测试结构数量。],
  [`... reset ...`],
  [其他], [`1`],
  table.cell(rowspan: 5)[`/time`], [`... query ...`], [查询的时间值、游戏时长或天数。],
  [`... pause`], table.cell(rowspan: 3)[`1`],
  [`... rate`],
  [`... resume`],
  [其他], [命令执行后的游戏时间，单位为游戏刻。],
  [`/title`], [], [指定的玩家总数。],
  [`/trigger`], [], [命令执行后分数持有者在记分项上的分数。],
  [`/version`], [], [`1`],
  table.cell(rowspan: 2)[`/waypoint`], [`... list`], [列出的路径点数量],
  [其他], [`0`],
  [`/weather`], [], [指定天气的持续时间，单位为的游戏刻数。若未指定 `<duration>`，则为 `-1`。],
  table.cell(rowspan: 8)[`/worldborder`], [`... add ...`], table.cell(rowspan: 3)[`<distance>`],
  [`... damage buffer ...`],
  [`... warning distance ...`],
  [`... center`], [`0`],
...
==== 存储于方块实体
语法为
#codebox("store (result|success) block <targetPos> <path> (int|float|short|long|double|byte) <scale> -> execute")
#param-desc(
  [`<targetPos>`（方块坐标 `minecraft:block_pos`）], [需要将返回值存入的方块坐标。],
  [`<path>`（NBT路径 `minecraft:nbt_path`）], [需要将返回值存入的NBT路径。],
  [`(int|float|short|long|double|byte)` ], [以何种数据类型存储数据，有 `byte`（字节型）、`short`（短整型）、`int`（整型）、`long`（长整型）、`float`（单精度浮点数）和 `double`（双精度浮点数）这些可用值。],
  [`<scale>`（双精度浮点数 `brigadier:double`）], [将返回值存储到NBT中时乘以的倍率。]
)
#example(
  [将假名 `#test` 在记分项 `[origin]` 上的分数存储为位于$(0,70,0)$的箱子中白色羊毛的数量。],
  [
    首先要获取假名 `#test` 在记分项 `[origin]` 上的分数：
    #codebox("scoreboard players get #test origin")
    #h(-2em)其次需要找到箱子NBT中存储白色羊毛数量的标签：
    #codebox("Items[{id:\"minecraft:white_wool\"}].count")
    #h(-2em)在存储数据的时候，由于 #icon("nbt-int") `count` 标签为整型，所以 `(int|float|short|long|double|byte)` 参数最好写成 `int`。完整的命令为
    #codebox("execute store result block 0 70 0 Items[{id:\"minecraft:white_wool\"}].count int 1.0 run scoreboard players get #test origin")
  ]
)
==== 存储于实体
语法为
#codebox("store (result|success) entity <target> <path> (int|float|short|long|double|byte) <scale> -> execute")
#param-desc(
  [`<target>`（实体 `minecraft:entity`）], [需要将返回值存入的实体，需要为玩家名、目标选择器或UUID，但是不能修改玩家的数据，且必须只能选择一个实体。]
)
和命令 `/data` 一样，这条命令也不能用于修改玩家的NBT。
#example(
  [将假名 `#test` 在记分项 `[origin]` 上的分数按0.1的缩放倍率存储为所有僵尸速度属性的基值。],
  [
    这个例子的难点在于找到存储僵尸速度属性的NBT，查阅节@sec:attribute 相关的内容，首先应找到拥有标签 `id:"minecraft:movement_speed"` 的 #icon("nbt-list") `attributes` 列表元素，再在次级节点中指定该复合标签内的 #icon("nbt-double") `base` 标签。合理的NBT路径为
    #codebox("attributes[{id:\"minecraft:movement_speed\"}].base")
    #h(-2em)标签 #icon("nbt-double") `base` 的类型是双精度浮点数，最好将存储数据类型也写为双精度浮点数，完整的命令为
    #codebox("execute as @e[type=zombie] store result entity @s attributes[{id:\"minecraft:movement_speed\"}].base double 0.1 run scoreboard players get #test origin")
  ]
)
==== 存储于命令存储
语法为
#codebox("store (result|success) storage <target> <path> (int|float|short|long|double|byte) <scale> -> execute")
#param-desc(
  [`<target>`（命名空间ID `minecraft:resource_location`）], [需要将返回值存入的命令存储的命名空间ID。]
)
#example(
  [将假名 `#test` 在记分项 `[origin]` 上的分数存储在命令存储 `custom:test` 内标签 #icon("nbt-int") `a` 的值，数据以整型存储。],
  [
    命令为
...
====== 信号强度作差
当比较器处于减法模式时，比较器会计算主输入$I$与两个侧输入中强度较大者的差，若小于0，则输出$O$为0；若大于0，则输出$O$为计算得到的差。公式为
$ O = max(0,I-max(L,R)) $
#example(
  [
    分别计算下列红石比较器输出端$O$的信号强度。
    #general-table(
      caption: "",
      colspan: 4,
      columns: (auto, auto, auto, auto),
      header: ([序号], [主输入端$I$], [侧输入端$L$], [侧输入端$R$]),
      [(1)], [9], [0], [3],
      [(2)], [6], [4], [6],
      [(3)], [5], [8], [11],
      [(4)], [7], [1], [2],
  ...
  [
    ...
    #general-table(
      ...
      colspan: 4,
      columns: (auto, auto, auto, auto),
      header: ([序号], [$L$与$R$中的较大值], [作差结果], [输出$O$]),
      [(1)], [3], [6], [6],
      [(2)], [6], [0], [0],
      [(3)], [11], [$-6$], [10],
      [(4)], [2], [5], [5],
...
====== 检测方块状态、方块实体和物品展示框
红石比较器可以将主输入端$I$的一些方块视作电源。如果主输入端$I$是一个受测方块，则红石比较器会检测该受测方块的状态或内容物，从而识别一定强度的信号，理论上红石比较器可识别的信号强度最大可至2147483647；如果红石比较器与受测方块之间隔了一个被充能的红石导体，则比较器的主输入$I$一定为15。

鉴于本章讨论的主题是命令方块电路，红石比较器在此主题的应用主要是检测命令方块，检测机制详见@sec:command_block。
==== 机械元件
机械元件是一类能够作出一定响应的方块。同样地，本章节只讨论命令方块这一种机械元件。
=== 常用电路分类
红石电路构建方式复杂、用途广泛，社区通常将其分为数字电路、模拟电路和生存电路等若干领域，而命令社区的玩家主要涉及的领域则是命令方块电路。虽然这些领域的研究方向不同，构成这些电路的一些基本构建方式却是固定的，下文对这些基本电路形式作出分类：
==== 脉冲电路
#proper-noun(display: "脉冲电路（Pulse circuit）", "mai4 chong1 dian4 lu4")，顾名思义，是一类可以产生、感应或改变脉冲的电路。脉冲电路的设计多种多样，可以产生不同长度的脉冲。具体设计应按需而定。
==== 时钟电路
#proper-noun(display: "时钟电路（Clock circuit）", "shi2 zhong1 dian4 lu4")是一类可以周期性产生脉冲的电路。

命令方块电路中常用的时钟电路为中继器时钟，中继器时钟是一个由红石中继器构成的首尾相接的环，当外部有脉冲输入后，此脉冲会沿着电路循环传输。中继器时钟的周期是环内所有中继器延迟的和。

输入时钟的脉冲长度不能超过此电路的周期，否则该时钟内的中继器会被永久激活从而无法传输脉冲，此时时钟电路必须重新建造。脉冲输入的方式可以是手动，也可以是自动。
#example(
  [创造一个以10秒为周期的中继器时钟多少红石中继器？],
  [
    10秒即100 rt，一个红石中继器最多可提供4 rt的延迟，因此至少需要$100 div 4 = 25$个红石中继器。

  ]
)
==== 逻辑电路
#proper-noun(display: "逻辑电路（Logic circuit）", "luo2 ji2 dian4 lu4")会判断输入信号是否满足逻辑，满足则产生特定输出。常用的逻辑电路有以下几种：
===== 非门
#general-table(
  caption: "非门真值表",
  colspan: 2,
  columns: (auto, auto),
  header: ([输入], [输出]),
  table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_red)[#text(fill: white)[关]],
  table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_green)[#text(fill: white)[开]]
)
非门会对输出信号进行反转，常用红石火把制作。
#figure(
  caption: "非门的设计",
  image("图片/非门的设计.png", width: 12em)
)
===== 或门
#general-table(
  caption: "或门真值表",
  ...
  columns: (auto, auto, auto),
  header: ([输入A], [输入B], [输出]),
  table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_red)[#text(fill: white)[关]],
  table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_green)[#text(fill: white)[开]],
  table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_green)[#text(fill: white)[开]],
  table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_green)[#text(fill: white)[开]]
)
对于或门：在所有输入中只要有输入为1，输出就为1。
#figure(
  caption: "或门的设计",
  image("图片/或门的设计.png", width: 6em)
)
===== 与门
#general-table(
  caption: "与门真值表",
  ...
  header: ([输入A], [输入B], [输出]),
  table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_red)[#text(fill: white)[关]],
  table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_red)[#text(fill: white)[关]],
  table.cell(fill: dark_red)[#text(fill: white)[关]], table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_red)[#text(fill: white)[关]],
  table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_green)[#text(fill: white)[开]], table.cell(fill: dark_green)[#text(fill: white)[开]]
)
对于或门：所有输入为1时，输出才为1。否则输出为0。
#figure(
  caption: "与门的设计",
  image("图片/与门的设计.png", width: 14em)
)
==== 记忆电路
记忆电路包括*锁存器*、*触发器*，可用于存储输入的数据。主要的记忆电路有RS或非锁存器、D触发器、JK触发器、T触发器等。
== 命令方块 <sec:command_block>
对于冒险地图的制作者和服务器管理员而言，为了系统地执行一些命令，减少在聊天栏中使用技术性命令的频率以实现命令执行的自动化，最需要的是一种能够用于输入、存储以及执行命令的媒介，而这种媒介就是于12w32a加入的命令方块。#proper-noun(display: "命令方块（Command block）", "ming4 ling4 fang1 kuai4")*是一种有指向的、能用于执行命令的方块。*

命令方块有多种获取方式，包括但不限于使用选取方块键（默认为 `鼠标中键`）复制已存在的命令方块或者在聊天栏中键入以下任意一条命令来获得命令方块：
#codebox("/give @s command_block")
#codebox("/setblock <x> <y> <z> command_block")
在快照22w44a之后，若启用按键选项中的管理员用品标签页，则命令方块还能够在创造模式物品栏“管理员用品”中获取。

命令方块只能在拥有管理员权限的情况下获取。值得注意的是，能否由命令获得命令方块不取决于玩家的游戏模式，只取决于玩家自身的权限等级。因此，只要有足够的权限，处于生存模式和冒险模式的玩家仍然通过命令可以获得命令方块，但是处于这些游戏模式下的玩家无法打开命令方块GUI。
=== 命令方块的使用
要想在命令方块中输入或修改命令，需要对命令方块点击使用键（默认为 `鼠标右键`）以打开其GUI。命令方块GUI的打开需要同时满足两个条件：一是玩家必须处于创造模式；二是玩家自身拥有2级或以上的权限等级。在没有足够权限的情况下，处于创造模式的玩家依旧无法放置、打开、使用命令方块或是破坏已放置的命令方块。
#figure(
  caption: "命令方块GUI",
  image("图片/命令方块GUI.png", width: 28em)
) <fig:command_block_gui_>
由@fig:command_block_gui_ 所示，可以将命令方块GUI大致分为若干个部分，从上至下依次为——控制台命令、上一个输出、功能区域和保存区域，下文将对这些部分进行具体的阐述：
==== 控制台命令
“控制台命令”输入框是玩家输入、修改命令的区域，这个区域可输入的字符最多为32500个，因此需要玩家将命令长度控制在32500字符以内。文本框长度有限，每次只能显示命令的其中段，`鼠标左键` 或使用 `←`、`→` 键移动光标能够调整命令显示的位置。由于每次打开命令方块GUI时光标总显示在命令的末尾，因此若想更改命令的前面部分，需要大幅度调整光标的位置。

在控制台命令文本框中输入命令时，不同于在聊天栏键入命令，此时的斜杠前缀 `/` 不是必须的。当文本框中无任何内容时，下方会显示一个命令列表（如图@fig:command_block_gui_ 所示），通过调整鼠标滚轮能够调整命令列表显示的位置，按 `Tab` 键能够在输入命令时自动补全或选择命令的部分。

如果在文本栏中输入了无效的命令，则错误的部分会显示为红色，在命令方块第一次接收到命令执行信号时，在文本栏的下方会显示错误的部分位于的字符数，并于后方显示命令无效的原因。
#figure(
  caption: "图中的错误部分应写作文本组件，故有如图所示的报错",
  image("图片/图中的错误部分应写作文本组件，故有如图所示的报错.png", width: 28em)
) <fig:command_block_output>
==== 上一个输出
带有字符 `O` 的按钮为控制是否显示上一个输出的按钮，它设置输出将显示时为 `O`，关闭时显示为 `X`。注意，显示输出会占用一定内存。当一个存档中有大量命令方块时，可以关闭输出显示以节省内存占用。每一次返回输出都只会显示一行字符。

@fig:command_block_gui_ 为命令方块的初始状态，由于没有上一个输出，因此该位置不显示任何内容。在命令方块内输入如@fig:command_block_output 所示的命令，由于 `ue` 不是文本组件，因此该命令实际为无效内容。

给予命令方块一个命令执行信号，再次打开命令方块GUI时可以看到显示输出的部分多了一个文本框，该框中的内容只可读不可编辑。文本框明显显示了上一次输出的具体时间，并对命令无效的地方进行了具体标识。若上一次命令执行正常且无任何错误，则该处位置不显示文本框。
==== 功能区域
该区域包含了三个不同的按钮，分别用于修改命令方块类型、制约模式和红石控制的模式。
===== 命令方块类型
第一个按钮用于修改命令方块类型，共有三种不同类型可供选择：脉冲型命令方块、连锁型命令方块和循环型命令方块。每种命令方块都有其独立的命名空间ID。
#figure(
  caption: "所有不同种类的命令方块：从左至右依次为脉冲型、连锁型、循环性；上方一排为不受制约的命令方块，下方一排为条件制约的命令方块",
  image("图片/所有不同种类的命令方块.png", width: 9em)
)
*脉冲型命令方块（Impulse command block，简称ICB）*#index(display: "脉冲型命令方块（Impulse command block，ICB）", "mai4 chong1 xing2 ming4 ling4 fang1 kuai4")通称命令方块，为命令方块的默认类型，其外观为橙色。ICB每激活一次就执行一次命令。

*连锁型命令方块（Chain command block，简称CCB）*#index(display: "连锁型命令方块（Chain command block，CCB）", "lian2 suo4 xing2 ming4 ling4 fang1 kuai4")外观为绿色。CCB仅在指向它的命令方块执行命令时才会执行一次命令，这个指向它的其他命令方块不必为CCB，也可以是ICB、RCB。已在同一游戏刻内执行过命令的CCB若再一次收到信号则不会在该游戏刻内再次执行命令。

*循环型命令方块（Repeating command block，简称RCB）*#index(display: "循环型命令方块（Repeating command block，RCB）", "xun2 huan2 xing2 ming4 ling4 fang1 kuai4")在被激活时每游戏刻执行一次命令，至不再激活为止，这种命令方块外观为紫色。
===== 制约模式
当一个连锁型命令方块处于条件制约模式时：上一个命令方块即指向它的命令方块成功执行命令后才会执行命令；当一个连锁型命令方块处于不受制约模式时：无论上一个命令方块是否成功执行命令都能够尝试执行一次命令。

*注意：条件制约只在连锁型命令方块中起作用，不可在脉冲型或循环型命令方块中应用条件制约，否则命令方块将会不工作。*
===== 红石控制
红石控制：必须接收到红石信号才能执行命令。

保持开启：不需要接收红石信号便能保持开启。但是这个设置对于不同类型的命令方块而言效果是不同的，脉冲型命令方块只会在命令保存、关闭GUI时执行一次命令，此后便不再执行命令；连锁型命令方块将会尝试持续感应由上一个命令方块传递的命令执行信号；循环型命令方块会自动在每一游戏刻执行一次命令。
==== 保存部分
点击 `完成` 按钮或按 `Enter` 以保存命令并退出命令方块GUI，点击 `取消` 按钮或按 `Esc` 在不保存任何修改的情况下退出命令方块GUI。
=== 命令方块链
上文在描述连锁型命令方块的概念时，引入了一个新的概念——命令方块链。*命令方块链是由多个命令方块首尾相连组成的有指向的链状结构。*
#figure(
  caption: "命令方块链",
  image("图片/命令方块链.png", width: 15em)
)
一个有效的命令方块链其链首不能为CCB，因为如果没有其他命令方块指向某CCB，则该CCB不会执行命令。链首可以是ICB或RCB。

命令方块的指向即为输出命令执行信号的方向，任何一种命令方块都能够将命令执行信号传递至下一个（即其指向的）命令方块，但是只有连锁型命令方块能够接收来自上一个（即指向其的）命令方块的命令执行信号。只有当传递信号——接受信号这一过程发生时，才可以认定为一个有效的命令方块链。
#example(
  [
    如@fig:command_block_chain_example_1 所示是命令方块链组合结构，所有命令方块中的命令语法均正确，有命令执行的信号输入左边第一个命令方块，求成功执行命令的命令方块个数。
    #figure(
      caption: "",
      image("图片/命令方块链例题1.png", width: 21em)
    ) <fig:command_block_chain_example_1>
  ],
  [
    给命令方块从左至右依次编号1\~7，则1\~3号命令方块都能成功执行命令。3号命令方块指向了4号命令方块，因此4号命令方块也能成功执行命令。由于4号命令方块不指向5号命令方块，因此5\~7号命令方块不执行命令，所以成功执行命令方块的个数为4。

    不难发现在3号命令方块将信号传递至4号命令方块时，4号命令方块的指向被忽略了，即使4号命令方块指向了3号命令方块。这是因为命令方块能够接收的信号可以来自六个不同的方向，而不仅仅是命令方块指向的相反方向。
  ]
)
#example(
  [
    在左右两个命令方块链结构中命令执行信号无法从左上方传递至右下方的是哪一个？
    #figure(
      caption: "",
      image("图片/命令方块链例题2.png", width: 21em)
    )
  ],
  [
    左边的结构在左下角部分不构成命令方块链，命令方块没有指向下一个命令方块，因此无法传递信号。

  ]
)
==== 命令方块链的逻辑特性
上文讲到，命令方块能够接收的信号可以来自六个不同的方向。之前所讲的都是只有一个方向输入信号的情况，下面来讨论含有多个方向输入信号的情况。

如@fig:command_block_chain_or，连锁型命令方块被两个脉冲型命令方块所指向，拥有两个信号输入端。但在实际情况中，连锁型命令方块只需要接收到一个命令执行信号即可尝试执行一次命令，因此其具有或门的特性：在任意一个输入为1时，输出为1。这样的特性在具有多个信号输入端时仍成立。
#figure(
  caption: "命令方块链的“或”门结构",
  image("图片/命令方块链的“或”门结构.png", width: 9em)
) <fig:command_block_chain_or>
#example(
  [
    两个红石块被同时放置在如@fig:command_block_chain_example_3 (a) 所示的位置，若所有命令方块中的命令语法均正确，则成功执行命令的命令方块个数为#blank。
    #sub-figure(
      caption: [],
      label: <fig:command_block_chain_example_3>,
      [#image("图片/命令方块链例题3-1.png", width: 14em)\(a)],
      [#image("图片/命令方块链例题3-2.png", width: 14em)\(b)]
    )
  ],
  [
    @fig:command_block_chain_example_3 (b) 清楚地展示了信号传递的路径，故成功执行命令的命令方块个数为10。

  ]
)
==== “链信号”的性质
@fig:command_block_chain_example_1、@fig:command_block_chain_example_3 展示了一种特殊的命令方块链结构：可以观察到，在这两个结构中，有一部分命令执行信号经由传递路径会回到上游的命令方块，即信号传递路径构成了闭合的环——@fig:command_block_chain_example_1 的3号命令方块将信号传递至4号命令方块后，4号命令方块同时也会将信号回传至3号命令方块；@fig:command_block_chain_example_3 (b) 的路径清楚地展现出了一个闭合环。然而在实际的操作中，这些接收两次信号的命令方块并不会将命令执行两次，这是因为*命令方块链中的所有命令方块将在同一游戏刻执行命令，经由路径传递的信号无法传递给一个已经在这个游戏刻执行过命令的方块。*

假如一个命令方块链中有一条命令控制了链的通断，比如使用 `/setblock` 命令覆盖链中某一位置的命令方块，既然信号传递具有瞬时性，那么在这种特殊情况下的信号传递又是怎样的呢？下面分三种情况讨论。
#figure(
  caption: "",
  [
    #box(image("图片/命令方块链通断1.png", width: 12em)) (a)\
    #box(image("图片/命令方块链通断2.png", width: 12em)) (b)\
    #box(image("图片/命令方块链通断3.png", width: 12em)) (c)\
  ]
)
第一种情况如图 (a) 所示，命令 `/setblock ~ ~ ~ air` 将执行该命令的命令方块替换为了空气方块。实际操作后，发现最右端的命令方块仍可以接收到信号。可以认定最中间的命令方块在将自身替换为空气方块的同时向下一个命令方块输出了信号。

第二种情况如图 (b) 所示，命令 `/setblock ~ ~ ~1 air` 将指向该命令方块的命令方块替换为了空气方块。实际操作后，发现最右端的命令方块也可以接收到信号。

第三种情况如图 (c) 所示，命令 `/setblock ~ ~ ~-1 air` 将该命令方块指向的命令方块替换为了空气方块。实际操作后，却发现最右端的命令方块无法接收到信号，这是由于中间的命令方块将其指向的命令方块替换为空气时，信号在这个地方被阻断了。这是因为*信号的传递虽具有瞬时性，但这些信号的传递在逻辑上是有先后顺序的，这个顺序由命令方块链的指向决定。*
=== 命令方块的信号输入与输出
==== 信号输入
命令方块有*1 gt*的延迟，当它受到上升沿时将在*1 gt*后尝试执行其中的命令。由充能理论，无论一个实体方块是被强充能还是被弱充能，都能够激活毗邻的命令方块。同时根据激活理论，被弱充能的方块无论信号等级均能够激活毗邻的命令方块。
#sub-figure(
  caption: [(a) 和 (b) 展示了两种充能：(a) 中的砂岩被强充能，能够激活毗邻的命令方块；(b) 中的砂岩被弱充能，也能够激活毗邻的命令方块。同时 (b) 中红石粉的信号强度为15，该强度的大小不会影响命令方块的激活],
  [#image("图片/命令方块充能1.png", width: 12em)\(a)],
  [#image("图片/命令方块充能2.png", width: 12em)\(b)]
)
#example(
  [
    判断这个命令方块能否被激活。
    #figure(
      caption: "",
      image("图片/命令方块信号输入输出例题.png", width: 9em)
    )
  ],
  [
    红石块能够向红石比较器输入信号，信号由比较器传输至命令方块，因此命令方块能够被激活。

  ]
)
一些情况下，当需要激活命令方块的时候，习惯上选取的传输元件是红石中继器，红石中继器会强充能其指向的方块，而红石粉只能弱充能，为此选择能够强充能毗邻方块的元件，于是电路中的延迟不可避免。红石粉提供了瞬时传输电路的功能，但其唯一的缺点是信号最多只能延伸15格。若传输的距离大于15格，则延迟依旧是不可避免的。
#sub-figure(
  caption: [命令方块无法被激活的情况],
  label: <fig:command_block_not_activated>,
  [#image("图片/命令方块无法被激活的情况1.png", width: 6em)\(a)],
  [#image("图片/命令方块无法被激活的情况2.png", width: 8em)\(b)]
)
@fig:command_block_not_activated 展示了一种常见的错误：红石粉会优先连接毗邻的红石中继器（如图 (a)），不会自动指向命令方块，在这种情况下的信号只是“经过”了命令方块，而没有激活命令方块。为了解决这种办法，可以在命令方块和红石粉之间加一个中继器（如图 (b)）或添加一条红石线，多余的延迟和空间占用是不可避免的。

红石中继器制造延迟的功能也较为常用，一些剧情类的冒险地图通常需要大量的红石中继器制造命令运行的时间差，下面举一个应用的例子：
#example(
  [在一个由中继器线构成的冒险地图剧情触发器中，剧情文字1、2分别在信号进入输入端后的1.2秒和3.4秒触发，求至少需要的红石中继器数量。],
  [
    可以很快构思出这条中继器线的基本结构：
    #figure(
      caption: "",
      image("图片/命令方块延迟线路结构.png", width: 20em)
    )
    这样的结构被广泛应用于剧情触发器中，用于控制剧情触发的时间。定义每处红石粉为一个“节点”，连接红石粉和命令方块的中继器为“节点中继器”。本题要求有2处剧情，需要2个这样的“节点”和“节点中继器”以及2段中继器线。但信号从中继器主线路传输到命令方块时，一定要经过“节点中继器”。但是“节点中继器”的延迟不会影响主线路，但会影响信号输入至到达命令方块的时间差。
    #figure(
      caption: "命令方块布置结构思路",
      image("图片/命令方块布置结构思路.png", width: 15em)
    )
    设节点1、2、3处的节点中继器延迟分别为$m$、$n$（$m$、$n in {1,2,3,4}$），这里的节点中继器通常只取1个）rt，则段1的总延迟为$(12-m)$rt，段2总延迟为$[34-n(12-m)]=[22+(m-n)]$rt。可以观察到，当$m$的值越大、$(m-n)$的值越小时，主线路中的延迟越短，即红石中继器的数量越少。则令$m=4$，$n=4$，段1延迟8 rt，段2延迟22 rt，最少总计$2+6+2=10$个中继器。

    如果去掉节点中继器：
    #figure(
      caption: "",
      image("图片/命令方块延迟线路结构2.png", width: 20em)
    ) <fig:command_block_delay_circuit_2>
    这样即可减去节点中继器延迟对主线路延迟的影响，使得主线路延迟计算更方便。红石中继器在激活命令方块的同时对命令方块进行了强充能，强充能的命令方块能够激活背对着它的红石中继器，因此能够让信号传递下去，而命令方块的指向不做要求。这种布置方式极大地节省了空间。此时中继器数量为$2+6=8$个。
  ]
)
==== 信号输出
命令方块不仅可以接受红石信号的输入，同时也可以输出红石信号。命令方块在成功运行其命令后会对任何背对着它的红石比较器（即命令方块位于红石比较器指向方向的相反方向）输出信号。当命令方块向比较器输出信号后，比较器后侧的火把会变亮。下表罗列了与之相关的所有结构，并指出了其中哪些结构是有效的：
#general-table(
  caption: "命令方块的信号输出",
  colspan: 2,
  columns: (auto, auto),
  header: ([结构], [解析]),
  image("图片/命令方块的信号输出1.png", width: 6em), [-],
  image("图片/命令方块的信号输出2.png", width: 6em), [在这种情况下由于红石比较器没有背对着命令方块，因此命令方块无法向红石比较器输出信号。],
  image("图片/命令方块的信号输出3.png", width: 6em), [虽然命令方块没有指向红石比较器，但由于比较器背对着命令方块，因此命令方块仍能向比较器输出信号。]
)
输出信号的信号强度与命令执行的成功次数有关。大多数命令执行的成功次数为1或0，部分命令的成功次数可以大于1，比如具有多分支的 `/execute`。

*命令方块会存储最后的执行成功次数直到再次执行命令，此期间红石比较器会持续输出信号。若再次执行命令后成功次数不改变，则红石比较器的信号将保持相同的输出。*
#example(
  [
    右上角标有红石块的命令方块为保持开启，画有“×”号的命令方块内命令执行失败，下面结构中成功执行命令的命令方块个数及后侧火把变亮的红石比较器个数为何？
    #figure(
      caption: "",
      image("图片/命令方块信号输出例题.png", width: 18em)
    )
  ],
  [
    由上节命令方块链有关知识可得，从左至右第五个命令方块由于条件限制，因此无法执行命令，同时也无法输出信号；第四个命令方块因语法错误也无法输出信号。故成功执行命令的命令方块个数为3，后侧火把变亮的红石比较器个数为3。

  ]
)
== 命令方块矿车 \*
整个命令系统除了有命令方块作为执行命令的工具外，还有命令方块矿车能用于执行命令。#proper-noun(display: "命令方块矿车（Minecart with command block）", "ming4 ling4 fang1 kuai4 kuang4 che1")*是一种装着脉冲型命令方块的矿车*，注意矿车只能装载脉冲型命令方块，无法装载连锁型和循环型命令方块，而且矿车中的命令方块无法修改功能。
#figure(
  caption: "命令方块矿车",
  image("图片/命令方块矿车.png", width: 10em)
)
和命令方块一样，命令方块矿车也可以在创造模式物品栏中获得。但是它与其他种类的矿车不同，无法由矿车和命令方块合成。但命令方块矿车可以通过命令 `/give @s command_block_minecart` 获得。由于矿车是*实体*，也可以使用 `/summon command_block_minecart` 生成。在生存模式或冒险模式中，击打命令方块矿车只会掉落矿车。

命令方块矿车结合了命令方块和矿车的性质，这使得它同时具备了执行命令和通过铁轨输出信号的功能。由于矿车本身的实体特性，它无法直接由一般的方块红石元件激活，能用于激活命令方块矿车的红石元件是*激活铁轨*#icon("sprite-activator_rail")。

只有在激活铁轨被激活后，才能激活途径的命令方块矿车。当命令方块停靠在激活铁轨上时，命令方块会每4gt执行一次命令。

另外一种能应用在命令方块矿车上的铁轨是*探测铁轨*#icon("sprite-detector_rail")，顾名思义，是用于探测命令方块矿车是否成功执行命令的铁轨，其作用与红石比较器对命令方块的作用一致。在使用探测铁轨的时候须结合红石比较器输出信号。
== 结构方块 <sec:structure_block>
在制作冒险地图或数据包的过程中，为节省时间，通常需要复制大量的方块或实体及其位置信息。对于命令而言，可以使用 `/clone` 将源区域的结构复制到目标区域。但是命令 `/clone` 无法直观地表现源区域或是目标区域的大小和位置，导致复制的过程容易出现不可挽回的错误。而且命令 `/clone` 无法处理实体信息、无法在存档与存档之间实现复制。

对于这些一定区域内方块和实体信息的集合，游戏使用“结构”的概念并将这些信息存储在游戏内部的文件夹中，当需要这些信息时便可以调出来使用。*#proper-noun(display: "结构（Structure）", "jie2 gou4")存储了一定大小长方体区域内方块和实体的信息。*

为了创建、存储、调用结构，可以使用#proper-noun(display: "结构方块（Structure block）", "jie2 gou4 fang1 kuai4")。启用管理员用品标签页时可以直接从创造模式物品栏获取结构方块，也可以使用下面的命令：
#codebox("/give @s structure_block")
#codebox("/setblock <x> <y> <z> structure_block")
结构方块事实上也是一种红石机械元件，可以被红石信号激活。被红石信号激活的结构方块只会将结构存储在内存而不是文件中，因此这种方式存储的结构是临时性而不是永久性的。若结构方块在被激活时正加载结构，则结构会被正常加载。

一个结构的大小被限定在$48 times 48 times 48$。若一个结构的大小超出了这些限定，则无法处理该结构。由玩家创建的结构被默认存储在 #icon("folder") `<存档名称> > generated > <命名空间> > structure` 文件夹下，以 `.nbt` 的文件格式保存，在命令中使用命名空间ID表示。
=== 结构的存储
Java版的结构方块提供了储存、加载、角落和数据四种模式，其中储存和角落模式被应用于结构的存储，本小节将着重介绍这两种模式。
==== 保存模式
#proper-noun(display: "保存模式（Save mode）", "bao3 cun2 mo2 shi4")的结构方块用于决定源区域的大小和位置，同时规定结构的名称。是否保存实体的选项可用于决定处于被复制区域内的实体是否被一并保存到结构中。在保存模式GUI内可输入相对位置和结构大小参数。这些参数必须为正整数，其中相对位置参数可以为非正数。
#figure(
  caption: "保存模式GUI（左）、默认相对位置的保存模式结构方块及结构位置（右）",
  [
    #box(image("图片/保存模式GUI.png", height: 12em))
    #h(2em)
    #box(image("图片/默认相对位置的保存模式结构方块及结构位置.png", height: 12em))
  ]
) <fig:save_mode>
相对位置决定了被复制结构起始方块的位置，即被复制结构西北下角的方块。默认的相对位置参数为0、1、0，该参数设定起始方块位于结构方块正上方毗邻位置。被复制的区域固定以该起始方块为基准，往三个坐标轴正方向延伸不大于48格的距离。@fig:save_mode（右）直观地表示了默认相对位置设置的被复制区域，其中红、绿、蓝三条线的长度分别由结构大小参数决定。设结构方块坐标为$(a,b,c)$，相对位置参数分别为$x_1$、$y_1$、$z_1$，结构大小参数分别为$x_2$、$y_2$、$z_2$（$x_1$、$y_1$、$z_1 in {lambda divides lambda in ZZ, 1 lt.eq.slant lambda lt.eq.slant 48}$）。则被复制区域起始方块的坐标为$(a+x_1,b+y_1,c+z_1)$，东南上角的方块坐标为$(a+x_1+x_2-1,b+y_1+y_2-1,c+z_1+z_2-1)$，整个结构的区域为
$ cases(
  a+x_1 lt.eq.slant x lt.eq.slant a+x_1+x_2,
  a+y_1 lt.eq.slant y lt.eq.slant a+y_1+y_2,
  a+z_1 lt.eq.slant z lt.eq.slant a+z_1+z_2
) $
==== 角落模式与辅助测量
当复制一个较大的结构时，计算结构的长宽高往往变得很不方便。#proper-noun(display: "角落模式（Corner mode）", "jiao3 luo4 mo2 shi4")提供了一种不用计量结构大小的复制方法，即确定结构的对角方块（体对角线）以确定整个结构的大小和位置。该过程被称为*辅助测量*。
#figure(
  caption: "角落模式GUI（左）、角落模式的辅助测量（右）",
  [
    #box(image("图片/角落模式GUI.png", height: 12em))
    #h(2em)
    #box(image("图片/角落模式的辅助测量.png", height: 12em))
  ]
)
角落模式的结构方块仅充当辅助测量的角色，需要配合保存模式的结构方块使用，*辅助测量的实质仍然是通过保存模式的结构方块复制结构*。三维空间内长方体的大小和位置可以通过长方体的体对角线得到，体对角线会根据两个*结构名称相同*的角落模式结构方块之间的最短顶点距离确定。确定被复制区域后在任意位置放置*结构名称相同*的保存模式结构方块，点击 `探测结构大小和位置` 按钮，游戏会自动测算被复制区域的大小以及相对于保存模式结构方块的位置。辅助测量也可以通过仅使用一个角落模式结构方块和一个保存模式结构方块实现，此时保存模式的结构方块会将自己作为角落模式的结构方块测算结构大小。辅助测量得到的结构大小仍不能超过$48 times 48 times 48$的限定，各项参数也不能超出其限制。

分别设两个角落模式的结构方块坐标$(x_1,y_1,z_1)$、$(x_2,y_2,z_2)$，保存模式的结构方块坐标$(a,b,c)$。可得三个结构大小参数$abs(x_1-x_2)-1$、$abs(y_1-y_2)-1$、$abs(z_1-z_2)-1$。根据大小参数的范围，有$1 lt.eq.slant abs(x_1-x_2)-1 lt.eq.slant 48$，即$2 lt.eq.slant abs(x_1-x_2) lt.eq.slant 49$，同理$2 lt.eq.slant abs(y_1-y_2) lt.eq.slant 49$，$2 lt.eq.slant abs(z_1-z_2) lt.eq.slant 49$，这些范围限定了角落模式结构方块的位置。

由于被复制的区域固定以结构西北下角的方块为基准，因此起始方块的三个坐标值必定为结构区域内的最小值，起始方块的坐标可表示为$(min(x_1,x_2)+1,min(y_1,y_2)+1,min(z_1,z_2)+1)$，故三个相对位置参数分别为$min(x_1,x_2)-a+1$，$min(y_1,y_2)-a+1$，$min(z_1,z_2)-a+1$，该结构的区域可以表示为
$ cases(
  min(x_1,x_2) + 1 lt.eq.slant x lt.eq.slant min(x_1,x_2) + abs(x_1-x_2),
  min(y_1,y_2) + 1 lt.eq.slant y lt.eq.slant min(y_1,y_2) + abs(y_1-y_2),
  min(z_1,z_2) + 1 lt.eq.slant z lt.eq.slant min(z_1,z_2) + abs(z_1-z_2)
) $
==== 结构空位
空气种类的方块，包括洞穴空气和虚空空气均用于区分未被占用的空间。这些空气通常作为结构的一部分，占用结构的空间。将结构从源区域复制到目标区域时，这些空气也会被复制下来并且会替换目标区域原有的方块。

#proper-noun(display: "结构空位（Structure void）", "jie2 gou4 kong4 wei4")用于表示结构生成时不被替换的未被占用空间，即源区域的结构空位会使目标区域相对位置的方块不被空气方块替换。这样的功能类似于命令 `/clone` 的 `masked` 模式，只是 `masked` 模式忽略了源区域的所有空气方块，而结构空位可用于选择性地忽略部分未被占用空间。

如@fig:structure_void，左图为保存模式，其中蓝色为空气方块，淡粉色为结构空位。复制到右图的位置时，可以看到结构空位对应的位置上原本的方块并没有被替换，而空气方块对应的位置上原本的方块被替换为了空气方块。
#figure(
  caption: "",
  [
    #box(image("图片/结构空位1.png", height: 9em))
    #h(2em)
    #box(image("图片/结构空位2.png", height: 9em))
  ]
) <fig:structure_void>
在源区域放置结构空位，点击保存模式GUI内的 `显示隐形方块` 选项，可以渲染出原本不可见的结构空位，为一个淡粉色的小方块。该选项也可用于渲染其他隐形方块，如屏障和光源方块。四种隐形方块的物品形式和渲染外观如下所示：
#general-table(
  caption: "隐形方块",
  colspan: 5,
  columns: (auto, auto, auto, auto, auto),
  header: ([方块], [空气], [结构空位], [屏障], [光源方块]),
  [物品形式], [无], [#icon("sprite-structure_void")], [#icon("sprite-barrier")], [#icon("sprite-light_15")],
  [渲染外观], image("图片/空气.png", height: 3em), image("图片/结构空位.png", height: 3em), image("图片/屏障.png", height: 3em), image("图片/光源方块.png", height: 3em)
)
=== 结构的加载
#proper-noun(display: "加载模式（Load mode）", "jia1 zai3 mo2 shi4")提供了生成结构的功能。通过输入结构名称、相对位置参数，可以生成已保存在文件中的结构。相对位置参数决定了结构起始方块相对于加载模式结构方块的位置，结构以该起始方块为基准向三个坐标轴正方向延伸。加载实体选项可决定生成的结构中是否包含结构文件中储存的实体信息。
#figure(
  caption: "加载模式GUI",
  image("图片/加载模式GUI.png", width: 28em)
)
结构完整性和种子可以决定生成的结构中有多少方块会被加载，未被加载的部分以空气方块填充。结构完整性参数必须为不小于0且不大于1的值，若该参数为0，则结构的所有方块均不会生成；1为该参数的默认值，此时结构的所有方块都会被加载；若该参数介于0到1之间，则生成的结构中会发生部分方块的缺失。结构完整性参数决定了结构中每一个方块的生成概率。

若一个结构有$n$个方块，结构完整性参数为$t$（$0 lt.eq.slant t lt.eq.slant 1$），这意味着$n$个方块中每个方块的生成概率均为$t$，同时不生成该方块的概率为$1-t$。记生成的方块数量为$X$，则实际生成$x$个方块的概率为
$ P(X = x) = "C"^x_n t^x (1-t)^(n-x) $
由此可得期望值
$ E(X) = sum^n_(i=1) i P(X=i) = sum^n_(i=1) i "C"^i_n t^i (1-t)^(n-i) = n t $
若种子参数设定为不为0的值，则方块的加载会以种子决定，在相同的结构中，相同的种子值会生成相同完整性的结构。若种子参数设置为0，则方块的加载为随机事件。

生成的结构除在完整性上发生变换外，还可以在结构的朝向上发生变换。加载模式的结构方块提供了旋转、镜像两种变换方式，为防止玩家加载结构错误，加载模式提供了显示方块的选项供直观观察结构的位置，同时结构生成中加载选项需点击两次。第一次点击会显示结构的外部框线，第二次点击方可生成结构。

旋转变换即为 `0`、`90`、`180`、`270` 四个选项，默认的旋转角度为0。四个选项中每次只能选择一个，因此无法进行旋转角度的叠加。旋转变换不改变$y$坐标而只改变$x$坐标和$z$坐标，沿$y$轴负方向观察，旋转总是顺时针的。规定与起始方块$x$、$z$方块坐标相同的方块组成“方块轴”，整个结构是绕该轴旋转的。
#figure(
  caption: "旋转变换（左）与镜像变换（右）",
  [
    #box(image("图片/旋转变换.png", height: 10em))
    #h(2em)
    #box(image("图片/镜像变换.png", height: 10em))
  ]
)
镜像变换有无 `|`、左右翻转 `←→` 和前后翻转 `↑↓` 三种。对于左右翻转 `←→`，结构将以结构内$z$坐标最小的方块组成的方块墙为对称轴进行镜像；对于前后翻转 `↑↓`，结构将以结构内$x$坐标最小的方块组成的方块墙为对称轴进行镜像。

对于结构的变换而言，整体结构的变换伴随着结构内每一个方块的位置变换，因此可以在空间内构建一些向量，使这些向量指向结构内每一个方块。在结构发生变换的时候，这些向量也会发生变换。使用矩阵可以方便地表示这些向量的变换。
#figure(
  caption: "结构的旋转",
  image("图片/结构的旋转.png", width: 8em)
)
对于结构的旋转，可以忽略$y$坐标而直接计算二维坐标。在$x O z$平面上构建一条向量，向量尾部为同一个结构、同一个高度内$x$坐标和$z$坐标最小的方块，向量头部为待研究方块，记该向量$bold(alpha) = (x,z)$，向量旋转过一定角度$theta$后得到向量$bold(alpha') = (x',z')$。由@subsec:rotate_and_local_coordinates 小节中推导局部坐标的方法不难得到下式：
$ mat(x'; z') = mat(cos theta, -sin theta; sin theta, cos theta)mat(x; z) $
由此可得到任意角度的旋转矩阵，结构方块提供了 `90`、`180`、`270` 三种旋转角度，于是这三种角度的旋转矩阵分别为：#box(baseline: 30%, inset: (y: 0.5em))[$mat(0, -1; 1, 0)$]、#box(baseline: 30%, inset: (y: 0.5em))[$mat(-1, 0; 0, -1)$]、#box(baseline: 30%, inset: (y: 0.5em))[$mat(0, 1; -1, 0)$]。而旋转角度 `0` 所对应的矩阵为#box(baseline: 30%, inset: (y: 0.5em))[$mat(1, 0; 0, 1)$]，这是一个单位矩阵，因此旋转角度为0时不改变方块的坐标。

对于结构的镜像而言，若为左右翻转 `←→`，则只变换$z$坐标，$x$坐标不变，即：
$ cases(
  x' = x,
  z' = -z
) $
用矩阵表示：
$ mat(x'; z') = mat(1, 0; 0, -1)mat(x; z) $
同理可得前后翻转 `↑↓` 的矩阵：#box(baseline: 30%, inset: (y: 0.5em))[$display(mat(-1, 0; 0, 1))$]

以上描述的结构变换均为普通变换，它们的特点是：*仅应用一次旋转或镜像变换*。与普通变换相对的变换为复合变换，即*同时应用旋转和镜像变换*。注意，同种变换之间只能选定一种变换方式，例如在旋转变换中，无法通过同时应用旋转 `90`、`180` 以实现旋转 `270` 的效果。普通变换一共有5种（3种旋转变换与2种镜像变换），因此复合变换一共有$3 times 2 = 6$种。

若应用复合变换，则非同种变换之间遵循*先镜像*、*后旋转*的变换规则，即先对结构应用镜像变换，再对其应用旋转变换。设向量$bold(alpha)$、旋转对应矩阵$bold(A)$和镜像对应矩阵$bold(B)$，在变换过程中使用矩阵的*左乘*，先应用镜像变换得到$bold(B alpha)$，再应用旋转变换得到$bold(A)(bold(B alpha))$。由于矩阵有分配律，则$bold(A)(bold(B alpha))=(bold(A B))bold(alpha)$，则可以得到每种复合变换的矩阵$bold(A B)$。注意，矩阵不满足交换律，在很多情况下 ，因此不能使用$bold(B A)$以代替$bold(A B)$。

结构方块旋转 `90`、左右翻转 `←→` 所对应的矩阵为
...
#general-table(
  ...
  colspan: 5,
  columns: (auto, auto, auto, auto, auto),
  header: ([旋转], [0], [90], [180], [270]),
  [无], [$bold(E)$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(0, -1; 1, 0))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(-1, 0; 0, -1))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(0, 1; -1, 0))$],
  [左右翻转 `←→`], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(1, 0; 0, -1))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(0, 1; 1, 0))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(-1, 0; 0, 1))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(0, -1; -1, 0))$],
  [前后翻转 `↑↓`], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(-1, 0; 0, 1))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(0, -1; -1, 0))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(1, 0; 0, -1))$], box(baseline: 30%, inset: (y: 0.5em))[$display(mat(0, 1; 1, 0))$]
)
观察上表不难总结出与结构方块变换的几点推论：
+ *当且仅当结构不作任何变换时，结构保持原样。*
+ *仅应用左右翻转 `←→` 与同时应用旋转 `180`、前后翻转 `↑↓` 有相同的效果；仅应用前后翻转 `↑↓` 与同时应用旋转 `180`、左右翻转 `←→` 有相同的效果。*
+ *同时应用旋转 `90`、左右翻转 `←→` 与同时应用旋转 `270`、前后翻转 `↑↓` 有相同的效果；同时应用旋转 `90`、前后翻转 `↑↓` 与同时应用旋转 `270`、左右翻转 `←→` 有相同的效果。*
#example(
  [有一个结构方块位于$(70,160,49)$，设置结构相对位置参数 `12`、`8`、`15`，若未经任何变换加载的结构a中有一个方块$A$位于$(95,183,78)$，求应用旋转 `270`、前后翻转的结构a中方块$A'$的坐标。],
  [
    由结构方块的位置和相对位置参数可以得到结构西北方向棱的$x O z$坐标$(82,64)$，记结构a中与方块$A$位于同一高度内的西北角方块为$O$，则$O$的二维坐标为$(82,64)$，可得二维向量$arrow.r(O A) = (13, 14)^"T"$，应用复合变换得
    #math.equation(numbering: none, block: true)[$arrow.r(O A') = mat(0, 1; -1, 0)mat(-1, 0; 0, 1)mat(13; 14) = mat(14; 13)$]
    因此可得$A'$的二维坐标$(96,77)$，变换不影响$y$坐标，因此三维坐标为$(96,183,77)$。
  ]
)
=== 数据模式 \*
#proper-noun(display: "数据模式（Data mode）", "shu4 ju4 mo2 shi4")可以通过在切换模式的时候按住 `Alt` 键唤出，但是该模式的结构方块只在一些结构的自然生成过程中起到作用。这种结构方块会作为结构的一部分生成，在世界生成过程中运行代码中指定的函数，完毕后即被删除。它对于玩家而言没有任何实际作用，玩家无法手动使其工作。
#heading(level: 2, numbering: none, [附录I思考题与习题])
+ 判断下列说法是否正确。
  + 一个命令方块共有六个可供输入信号的方向。
  + 直接由命令给予的循环型命令方块默认为条件制约和保持开启。
  + 按功能区域划分，一共可以分出12种不同功能的命令方块。
+ 写出直接给予当前玩家循环型命令方块的命令。
+ 在下列命令方块链结构中，用红石块上标表示保持开启的命令方块，用“×”表示命令语法不正确的命令方块，部分结构的信号输入如箭头方向所指示或用红石块表示，其中箭头表示信号输入的方向，写出每个结构成功执行命令的命令方块个数。
  #grid(
    align: top,
    columns: (1fr, 1fr),
    row-gutter: 0.8em,
    [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(1)"), box(image("图片/命令方块链习题1.png", width: 8em)))], [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(2)"), box(image("图片/命令方块链习题2.png", width: 6em)))],
    [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(3)"), box(image("图片/命令方块链习题3.png", width: 8em)))], [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(4)"), box(image("图片/命令方块链习题4.png", width: 8em)))],
    [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(5)"), box(image("图片/命令方块链习题5.png", width: 8em)))], [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(6)"), box(image("图片/命令方块链习题6.png", width: 8em)))],
    [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(7)"), box(image("图片/命令方块链习题7.png", width: 8em)))], [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(8)"), box(image("图片/命令方块链习题8.png", width: 6em)))],
    [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(9)"), box(image("图片/命令方块链习题9.png", width: 10em)))], [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(10)"), box(image("图片/命令方块链习题10.png", width: 8em)))],
    [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(11)"), box(image("图片/命令方块链习题11.png", width: 8em)))], [#stack(dir: ltr, spacing: 0pt, box(width: 2em, "(12)"), box(image("图片/命令方块链习题12.png", width: 8em)))]
  )
+ 以下哪些结构无法激活命令方块？
  + 毗邻的红石块
  + 附着的红石火把
  + 毗邻的石质按钮
  + 毗邻的拉杆
+ 一个石质按钮毗邻了一个循环型红石控制的命令方块，控制台命令为 `/say a`，按下石质按钮后聊天栏中会出现的字符 `a` 的数量是#blank。
+ 在下面的结构中，每个中继器的档位已标出，不计卡顿，则按下按钮至信号到达最右边的命令方块所需的时间为#blank。
  #figure(
    caption: "",
    image("图片/中继器线路时间计算习题.png", width: 20em)
  )
+ 在如图所示的结构中红石中继器均为1档。
  #figure(
    caption: "",
    image("图片/中继器线路时间计算习题2.png", width: 14em)
  )
  + 控制台命令的语法均正确，则成功执行命令的命令方块个数为#blank。
  + 从信号输入至右下角命令方块被激活的延迟为#blank 游戏刻。
+ 使用@fig:command_block_delay_circuit_2 所示的剧情触发结构安排三段剧情文字，分别在信号进入输入端后的0.6秒、2.1秒和4.5秒触发，求至少需要的红石中继器数量。
+ 一个结构方块的坐标为$(35,17,-168)$，其相对位置参数为 `4`、`12`、`9`，结构大小参数为 `37`、`14`、`22`，试判断方块坐标$(48,37,-140)$和$(55,49,-133)$是否位于该结构内。
+ 若结构有7个方块，设置加载该结构时结构完整性参数为0.2，求生成方块个数的期望值。
+ 使用辅助测量进行结构储存时，在方块坐标$(235,65,-104)$处设置一个角落模式的结构方块，若要求被复制区域的大小参数为 `12`、`10`、`8`，求第二个角落模式的结构方块可以放置的所有位置。
+ 表示下列结构的区域。
  + 一个保存模式的结构方块坐标为$(30,36,72)$，设置相对位置参数 `-19`、`27`、`8`，大小参数 `32`、`16`、`32`；
  + 已知保存模式的结构方块坐标为$(-173,45,62)$，结构名称相同的角落模式结构方块坐标为$(-144,60,67)$；
  + 在保存模式中设置大小参数 `17`、`23`、`4` 的一个结构，使用加载模式的结构方块进行生成时，设置相对位置参数 `10`、`1`、`8`，且应用旋转 `270`、前后翻转 `↑↓`。加载模式的结构方块坐标为$(77,42,60)$。
+ 计算下列方块经变换后的坐标。
#general-table(
  caption: "",
  colspan: 5,
  columns: (auto, auto, auto, auto, auto),
  header: ([题号], [加载模式结构方块坐标], [相对位置参数], [结构内未经变换的方块坐标], [变换]),
  [(1)], [$(17,28,-16)$], [`15`、`22`、`30`], [$(40,67,23)$], [旋转 `270`],
  [(2)], [$(130,-10,42)$], [`-16`、`18`、`12`], [$(131,-10,42)$], [前后翻转 `↑↓`],
  [(3)], [$(-22,22,40)$], [`21`、`-13`、`-14`], [$(6,17,49)$], [镜像 `90`、左右翻转 `←→`],
  [(4)], [$(35,46,15)$], [`-10`、`6`、`9`], [$(33,70,45)$], [镜像 `180`、前后翻转 `↑↓`]
)
= 数据库
...
#split-table(
  ...
  data: (
    ...
    [26.1 Release Candidate 1], [101.1], [84.0],
    [26.1 Release Candidate 2], [101.1], [84.0],
...
此章节收录所有方块可用的方块状态（流体状态），带黑色底纹的为该属性的默认值。不在此附录中列出的方块没有方块属性。
#general-table(
  caption: "方块状态总表",
  colspan: 4,
  columns: (auto, auto, auto, auto),
  header: ([方块], [属性], [描述], [可用值]),
  table.cell(rowspan: 10)[激活铁轨、动力铁轨、探测铁轨], table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活或提供红石信号], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[`shape`], table.cell(rowspan: 6)[铁轨的爬坡、走向], [`ascending_east`], [`ascending_north`], [`ascending_south`], [`ascending_west`], [`east_west`], table.cell(fill: black)[`north_south`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[所有种类紫晶芽、紫水晶簇], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], [`north`], [`south`], table.cell(fill: black)[`up`], [`west`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[所有种类铁砧、结果的西瓜茎、结果的南瓜茎、切石机、南瓜灯、所有种类带釉陶瓦、墙上的旗帜、织布机、雕刻南瓜、墙上的火把、墙上的灵魂火把、墙上的铜火把], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 7)[竹子], table.cell(rowspan: 2)[`age`], table.cell(rowspan: 2)[竹子的生长阶段], table.cell(fill: black)[`0`], [`1`], table.cell(rowspan: 3)[`leaves`], table.cell(rowspan: 3)[竹子的树叶量], table.cell(fill: black)[`none`], [`small`], [`large`],
  table.cell(rowspan: 2)[`stage`], table.cell(rowspan: 2)[竹子是否不再生长，为 `0` 时会继续生长], table.cell(fill: black)[`0`], [`1`],
  [所有种类旗帜], [`rotation`], [方块的朝向，以 `0` 为南，值增大的顺序为顺时针。默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 8)[木桶], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], table.cell(fill: black)[`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 2)[`open`], table.cell(rowspan: 2)[木桶是否打开], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[所有种类床], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`occupied`], table.cell(rowspan: 2)[床是否被占用], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`part`], table.cell(rowspan: 2)[床头还是床尾], table.cell(fill: black)[`foot`], [`head`],
  table.cell(rowspan: 5)[蜂巢、蜂箱], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  [`honey_level`], [蜂蜜储量，默认为 `0`], [`0` \~ `5`],
  [甜菜根], [`age`], [农作物生长阶段，默认为 `0`], [`0` \~ `3`],
  table.cell(rowspan: 10)[钟], table.cell(rowspan: 4)[`attachment`], table.cell(rowspan: 4)[钟的附着方式，可悬挂、挂在两面墙之间、平地放置或挂在一面墙上], [`ceiling`], [`double_wall`], [`floor`], [`single_wall`],
  ...
  table.cell(rowspan: 10)[大型垂滴叶], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 4)[`tilt`], table.cell(rowspan: 4)[大型垂滴叶的倾斜程度], [`full`], table.cell(fill: black)[`none`], [`partial`], [`unstable`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[高炉、熔炉、烟熏炉], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`lit`], table.cell(rowspan: 2)[熔炉是否燃烧], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[酿造台], table.cell(rowspan: 2)[`has_bottle_0`], table.cell(rowspan: 2)[酿造台0号槽位是否有药水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`has_bottle_1`], table.cell(rowspan: 2)[酿造台1号槽位是否有药水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`has_bottle_2`], table.cell(rowspan: 2)[酿造台2号槽位是否有药水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[气泡柱], table.cell(rowspan: 2)[`drag`], table.cell(rowspan: 2)[气泡柱是否为涡流，否则为涌流], [`false`], table.cell(fill: black)[`true`],
  table.cell(rowspan: 9)[所有种类按钮、拉杆], table.cell(rowspan: 3)[`face`], table.cell(rowspan: 3)[方块附着在其他方块的面], [`ceiling`], [`floor`], table.cell(fill: black)[`wall`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否提供红石信号], table.cell(fill: black)[`false`], [`true`],
  [仙人掌、甘蔗], [`age`], [植物的生长阶段，为 `15` 时可以长出新的植物方块，默认为 `0`], [`0` \~ `15`],
  [蛋糕], [`bites`], [蛋糕被食用的次数，默认为0], [`0` \~ `6`],
  table.cell(rowspan: 2)[插上任意蜡烛的蛋糕、红石灯、红石矿石、深层红石矿石], table.cell(rowspan: 2)[`lit`], table.cell(rowspan: 2)[是否点亮], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 10)[校频幽匿感测体], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  [`power`], [红石信号等级，默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 3)[`sculk_sensor_phase`], table.cell(rowspan: 3)[幽匿感测体的触发情况], [`active`], [`cooldown`], table.cell(fill: black)[`inactive`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 10)[营火、灵魂营火], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`lit`], table.cell(rowspan: 2)[营火是否燃烧], [`false`], table.cell(fill: black)[`true`],
  table.cell(rowspan: 2)[`signal_fire`], table.cell(rowspan: 2)[营火是否用作信号营火、若是则烟雾更高], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 5)[所有种类蜡烛], [`candles`], [蜡烛的数量，默认为 `1`], [`1` \~ `4`],
  table.cell(rowspan: 2)[`lit`], table.cell(rowspan: 2)[蜡烛是否点亮], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  [胡萝卜、马铃薯], [`age`], [胡萝卜和马铃薯的生长阶段，其中`0` \~ `1` 为第一阶段，`2` \~ `3` 为第二阶段，`4` \~ `6` 为第三阶段，`7` 为成熟阶段。默认为 `0`], [`0` \~ `7`],
  table.cell(rowspan: 3)[洞穴藤蔓], [`age`], [植物的生长程度，小于 `25` 时可以生长出新的植物方块，且使得末端植物方块的该属性增加 `1`，当此值达到 `25` 时植物不再生长。默认为 `0`], [`0` \~ `25`],
  table.cell(rowspan: 2)[`berries`], table.cell(rowspan: 2)[是否带有发光浆果], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[洞穴藤蔓植株], table.cell(rowspan: 2)[`berries`], table.cell(rowspan: 2)[是否带有发光浆果], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 9)[箱子、陷阱箱、所有种类铜箱子], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 3)[`type`], table.cell(rowspan: 3)[箱子的类型，分为单个箱子、大箱子的左半部分和大箱子的右半部分], [`left`], [`right`], table.cell(fill: black)[`single`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 16)[雕纹书架], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`slot_0_occupied`], table.cell(rowspan: 2)[雕纹书架0号槽位是否有物品], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`slot_1_occupied`], table.cell(rowspan: 2)[雕纹书架1号槽位是否有物品], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`slot_2_occupied`], table.cell(rowspan: 2)[雕纹书架2号槽位是否有物品], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`slot_3_occupied`], table.cell(rowspan: 2)[雕纹书架3号槽位是否有物品], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`slot_4_occupied`], table.cell(rowspan: 2)[雕纹书架4号槽位是否有物品], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`slot_5_occupied`], table.cell(rowspan: 2)[雕纹书架5号槽位是否有物品], table.cell(fill: black)[`false`], [`true`],
  [紫颂花], [`age`], [紫颂花的生长状态，颂花横向生长或长出其他紫颂花时该属性的值增加，以 `0` 为刚被放置或生长出后的状态，`5` 为枯萎状态。默认值为 `0`], [`0` \~ `5`],
  table.cell(rowspan: 12)[紫颂植株], table.cell(rowspan: 2)[`down`], table.cell(rowspan: 2)[紫颂植株是否与下方的方块相连], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[紫颂植株是否与东方的方块相连], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[紫颂植株是否与北方的方块相连], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[紫颂植株是否与南方的方块相连], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`up`], table.cell(rowspan: 2)[紫颂植株是否与上方的方块相连], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[紫颂植株是否与西方的方块相连], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 5)[可可果], [`age`], [可可果的生长阶段，`2` 为成熟阶段，默认值为 `0`], [`0` \~ `2`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 8)[所有种类命令方块], table.cell(rowspan: 2)[`conditinal`], table.cell(rowspan: 2)[命令方块是否处于条件制约模式], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], table.cell(fill: black)[`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 8)[红石比较器], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`mode`], table.cell(rowspan: 2)[红石比较器的模式], table.cell(fill: black)[`compare`], [`subtract`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  [堆肥桶], [`level`], [堆肥量，默认为 `0`], [`0` \~ `8`],
  table.cell(rowspan: 4)[所有种类铜灯], table.cell(rowspan: 2)[`lit`], table.cell(rowspan: 2)[铜灯是否点亮], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 10)[所有种类铜傀儡像], table.cell(rowspan: 4)[`copper_golem_pose`], table.cell(rowspan: 4)[铜傀儡像的姿势], [`running`], [`sitting`], [`standing`], [`star`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[所有种类（失活的）珊瑚、（失活的）珊瑚扇、垂根、屏障、沉重核心、红树根、所有种类的铜格栅], table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[所有种类墙上的（失活）珊瑚扇], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 16)[合成器], table.cell(rowspan: 2)[`crafting`], table.cell(rowspan: 2)[合成器是否正在合成物品], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 12)[`orientation`], table.cell(rowspan: 12)[合成器的朝向，属性的值中下滑线前是方块面的朝向，下滑线后是顶部的朝向], [`down_east`], [`down_north`], [`down_south`], [`down_west`], [`east_up`], table.cell(fill: black)[`north_up`], [`south_up`], [`up_east`], [`up_north`], [`up_south`], [`up_west`], [`west_up`],
  table.cell(rowspan: 2)[`triggered`], table.cell(rowspan: 2)[合成器是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[嘎枝之心], table.cell(rowspan: 3)[`axis`], table.cell(rowspan: 3)[方块的轴向], [`x`], table.cell(fill: black)[`y`], [`z`],
  table.cell(rowspan: 3)[`creaking_heart_state`], table.cell(rowspan: 3)[嘎枝之心的状态，其中 `awake` 为激活，`dormant` 为休眠，`uprooted` 为无效], [`awake`], [`dormant`], table.cell(fill: black)[`uprooted`],
  table.cell(rowspan: 2)[`natural`], table.cell(rowspan: 2)[嘎枝之心是否为自然生成], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[阳光探测器], table.cell(rowspan: 2)[`inverted`], table.cell(rowspan: 2)[是否为反向阳光探测器], table.cell(fill: black)[`false`], [`true`],
  [`power`], [红石信号等级，默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 8)[饰纹陶罐], table.cell(rowspan: 2)[`cracked`], table.cell(rowspan: 2)[饰纹陶罐是否会碎裂为组成它的陶片，如果否，则掉落自身], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[发射器、投掷器], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], table.cell(fill: black)[`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 2)[`triggered`], table.cell(rowspan: 2)[发射器或投掷器是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 12)[所有种类门], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  ...
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 7)[失水恶魂], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  [`hydration`], [失水恶魂的湿润阶段，默认为 `0`], [`0` \~ `3`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[末地传送门框架], table.cell(rowspan: 2)[`eye`], table.cell(rowspan: 2)[末地传送门框架是否嵌入末影之眼], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 6)[末地烛、所有种类潜影盒], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], [`north`], [`south`], table.cell(fill: black)[`up`], [`west`],
  [耕地], [`moisture`], [耕地的湿润程度，`7` 为完全湿润，默认值为 `0`], [`0` \~ `7`],
  table.cell(rowspan: 11)[火], [`age`], [火的“年龄”，影响火的传播和熄灭随时间变化。默认值为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[是否附着在其他方块的东方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[是否附着在其他方块的北方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[是否附着在其他方块的南方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`up`], table.cell(rowspan: 2)[是否附着在其他方块的上方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[是否附着在其他方块的西方], table.cell(fill: black)[`false`], [`true`],
  [霜冰], [`age`], [霜冰碎裂的程度，`0` 为完全不碎裂。默认为 `0`], [`0` \~ `3`],
  table.cell(rowspan: 10)[所有种类栅栏、玻璃板、铁栏杆、所有种类铜栏杆], table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[是否与东面的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[是否与北面的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[是否与南面的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[是否与西面的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 10)[所有种类栅栏门], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`in_wall`], table.cell(rowspan: 2)[栅栏是否嵌入墙内], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`open`], table.cell(rowspan: 2)[门的开关], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 14)[发光地衣、幽匿脉络、树脂团], table.cell(rowspan: 2)[`down`], table.cell(rowspan: 2)[是否附着在其他方块的下方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[是否附着在其他方块的东方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[是否附着在其他方块的北方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[是否附着在其他方块的南方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`up`], table.cell(rowspan: 2)[是否附着在其他方块的上方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[是否附着在其他方块的西方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[草方块、灰化土、菌丝], table.cell(rowspan: 2)[`snowy`], table.cell(rowspan: 2)[是否覆雪], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 7)[砂轮], table.cell(rowspan: 3)[`face`], table.cell(rowspan: 3)[方块附着在其他方块的面], [`ceiling`], [`floor`], table.cell(fill: black)[`wall`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 5)[所有种类悬挂式告示牌], table.cell(rowspan: 2)[`attached`], table.cell(rowspan: 2)[锁链系于一点 `true` 还是相互平行 `false`], table.cell(fill: black)[`false`], [`true`],
  [`rotation`], [方块的朝向，以 `0` 为南，值增大的顺序为顺时针。默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[所有种类生物头颅], table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  [`rotation`], [方块的朝向，以 `0` 为南，值增大的顺序为顺时针。默认为 `0`], [`0` \~ `15`],
  [重质测重压力板、轻质测重压力板、标靶], [`power`], [红石信号等级，默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 8)[漏斗], table.cell(rowspan: 2)[`enabled`], table.cell(rowspan: 2)[漏斗是否可以吸取和传输物品], [`false`], table.cell(fill: black)[`true`],
  table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[漏斗的输出方向], table.cell(fill: black)[`down`], [`east`], [`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 5)[铁链、所有种类的铜链], table.cell(rowspan: 3)[`axis`], table.cell(rowspan: 3)[方块的轴向], [`x`], table.cell(fill: black)[`y`], [`z`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 12)[拼图方块], table.cell(rowspan: 12)[`orientation`], table.cell(rowspan: 12)[拼图方块的朝向，属性的值中下滑线前是方块面的朝向，下滑线后是顶部的朝向], [`down_east`], [`down_north`], [`down_south`], [`down_west`], [`east_up`], table.cell(fill: black)[`north_up`], [`south_up`], [`up_east`], [`up_north`], [`up_south`], [`up_west`], [`west_up`],
  [海带、垂泪藤、缠怨藤], [`age`], [植物的生长程度，小于 `25` 时可以生长出新的植物方块，且使得末端植物方块的该属性增加1，当此值达到 `25` 时植物不再生长。默认为 `0`], [`0` \~ `25`],
  table.cell(rowspan: 2)[大型蕨、高草丛、丁香、向日葵、牡丹、玫瑰丛、瓶子草、高海草], table.cell(rowspan: 2)[`half`], table.cell(rowspan: 2)[花的上下半部分], table.cell(fill: black)[`lower`], [`upper`],
  table.cell(rowspan: 4)[灯笼、灵魂灯笼、所有种类铜灯笼], table.cell(rowspan: 2)[`hanging`], table.cell(rowspan: 2)[灯笼是否悬挂在其他方块之下], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[熔岩、水], [`level`], [流体的流动情况，为 `0` 时是熔岩源或水源方块。默认值为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 2)[`falling`], table.cell(rowspan: 2)[流体是否向下流动], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 5)[枯叶堆], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  [`segment_amount`], [枯叶堆中枯叶的数量，默认为 `1`], [`1` \~ `4`],
  table.cell(rowspan: 5)[所有种类树叶], [`distance`], [离该树叶最近的原木距离，默认为 `7`], [`1` \~ `7`],
  table.cell(rowspan: 2)[`persistent`], table.cell(rowspan: 2)[树叶是否无视距离持久存在], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[讲台], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`has_book`], table.cell(rowspan: 2)[讲台内是否有书], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否提供红石信号], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[光源方块], [`level`], [光源方块的亮度等级。默认为 `15`], [`0` \~ `15`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 10)[所有种类避雷针], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], [`north`], [`south`], table.cell(fill: black)[`up`], [`west`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否提供红石信号], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[所有种类原木、木头、菌柄、菌核、去皮原木、去皮木头、去皮菌柄、去皮菌核、竹块、去皮竹块干草捆、粘泥的红树根、深板岩、玄武岩、所有种类蛙明灯、石英柱、磨制玄武岩、紫珀柱、虫蚀深板岩、骨块], table.cell(rowspan: 3)[`axis`], table.cell(rowspan: 3)[方块的轴向], [`x`], table.cell(fill: black)[`y`], [`z`],
  table.cell(rowspan: 8)[移动的活塞], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], table.cell(fill: black)[`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 2)[`type`], table.cell(rowspan: 2)[此活塞的类型], table.cell(fill: black)[`normal`], [`sticky`],
  table.cell(rowspan: 12)[蘑菇柄、棕色蘑菇方块、红色蘑菇方块], table.cell(rowspan: 2)[`down`], table.cell(rowspan: 2)[是否在底面使用蘑菇盖或柄的纹理，若为否，则使用气孔纹理], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[是否在东面使用蘑菇盖或柄的纹理，若为否，则使用气孔纹理], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[是否在北面使用蘑菇盖或柄的纹理，若为否，则使用气孔纹理], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[是否在南面使用蘑菇盖或柄的纹理，若为否，则使用气孔纹理], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`up`], table.cell(rowspan: 2)[是否在顶面使用蘑菇盖或柄的纹理，若为否，则使用气孔纹理], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[是否在西面使用蘑菇盖或柄的纹理，若为否，则使用气孔纹理], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[下界传送门], table.cell(rowspan: 2)[`axis`], table.cell(rowspan: 2)[下界传送门的朝向], table.cell(fill: black)[`x`], [`z`],
  [下界疣], [`age`], [下界疣的生长阶段，其中 `0` 为生长第一阶段，`1` \~ `2` 为生长第二阶段，`3` 为成熟阶段], [`0` \~ `3`],
  table.cell(rowspan: 30)[音符盒], table.cell(rowspan: 27)[`instrument`], table.cell(rowspan: 27)[音符盒的乐器], [`banjo`], [`basedrum`], [`bass`], [`bell`], [`bit`], [`chime`], [`cow_bell`], [`creeper`], [`custom_head`], [`didgeridoo`], [`dragon`], [`flute`], [`guitar`], table.cell(fill: black)[`harp`], [`hat`], [`iron_xylophone`], [`piglin`], [`pling`], [`trumpet`], [`trumpet_exposed`], [`trumpet_oxidized`], [`trumpet_weathered`], [`skeleton`], [`snare`], [`wither_skeleton`], [`xylophone`], [`zombie`],
  [`note`], [音符盒的音高，默认为 `0`], [`0` \~ `24`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[侦测器], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], [`north`], table.cell(fill: black)[`south`], [`up`], [`west`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否提供红石信号], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[苍白垂须], table.cell(rowspan: 2)[`tip`], table.cell(rowspan: 2)[下方是否有], [`false`], table.cell(fill: black)[`true`],
  table.cell(rowspan: 14)[苍白覆地苔藓], table.cell(rowspan: 2)[`bottom`], table.cell(rowspan: 2)[苍白覆地苔藓是否具有底部], [`false`], table.cell(fill: black)[`true`],
  table.cell(rowspan: 3)[`east`], table.cell(rowspan: 3)[苍白覆地苔藓是否在东面向上延伸、延伸的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 3)[`north`], table.cell(rowspan: 3)[苍白覆地苔藓是否在北面向上延伸、延伸的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 3)[`south`], table.cell(rowspan: 3)[苍白覆地苔藓是否在南面向上延伸、延伸的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 3)[`west`], table.cell(rowspan: 3)[苍白覆地苔藓是否在西面向上延伸、延伸的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 5)[粉红色花簇、野花簇], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  [`flower_amount`], [花簇中花的数量，默认为 `1`], [`1` \~ `4`],
  table.cell(rowspan: 8)[活塞、粘性活塞], table.cell(rowspan: 2)[`extended`], table.cell(rowspan: 2)[活塞头是否伸出], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], table.cell(fill: black)[`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 10)[活塞头], table.cell(rowspan: 6)[`facing`], table.cell(rowspan: 6)[方块的朝向], [`down`], [`east`], table.cell(fill: black)[`north`], [`south`], [`up`], [`west`],
  table.cell(rowspan: 2)[`short`], table.cell(rowspan: 2)[是否为较短的活塞头], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`type`], table.cell(rowspan: 2)[此活塞的类型], table.cell(fill: black)[`normal`], [`sticky`],
  ...
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[墙上的红石火把], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 9)[红石中继器], [`delay`], [红石中继器的挡位，默认为 `1`], [`1` \~ `4`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`locked`], table.cell(rowspan: 2)[红石中继器是否被锁存], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  [重生锚], [`charges`], [重生锚的能量等级，默认为 `0`], [`0` \~ `4`],
  table.cell(rowspan: 2)[除红树胎生苗外其他所有种类树苗], table.cell(rowspan: 2)[`stage`], table.cell(rowspan: 2)[生长阶段，为 `1` 时可以成长为树], table.cell(fill: black)[`0`], [`1`],
  table.cell(rowspan: 5)[脚手架], table.cell(rowspan: 2)[`bottom`], table.cell(rowspan: 2)[脚手架是否浮空而具有底部], table.cell(fill: black)[`false`], [`true`],
  [`distance`], [脚手架与最近可支撑脚手架的距离，为 `7` 时转变成下落的方块，默认为 `7`], [`0` \~ `7`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[幽匿催发体], table.cell(rowspan: 2)[`bloom`], table.cell(rowspan: 2)[幽匿催发体是否处于蔓延状态], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[幽匿感测体], [`power`], [红石信号等级，默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 3)[`sculk_sensor_phase`], table.cell(rowspan: 3)[幽匿感测体的触发情况], [`active`], [`cooldown`], table.cell(fill: black)[`inactive`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[幽匿尖啸体], table.cell(rowspan: 2)[`can_summon`], table.cell(rowspan: 2)[幽匿尖啸体是否可以召唤监守者], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`shrieking`], table.cell(rowspan: 2)[幽匿尖啸体是否正在发出尖啸], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[海泡菜], [`pickles`], [海泡菜数量，默认为 `1`], [`1` \~ `4`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 12)[所有种类展示架], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[`side_chain`], table.cell(rowspan: 4)[展示架的连接方式：`center` 为与两边连接，`left` 为与左边连接，`right` 为与右边连接，`unconnected` 为不连接], [`center`], [`left`], [`right`], table.cell(fill: black)[`unconnected`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[所有种类告示牌], [`rotation`], [方块的朝向，以 `0` 为南，值增大的顺序为顺时针。默认为 `0`], [`0` \~ `15`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 5)[所有种类台阶], table.cell(rowspan: 3)[`type`], table.cell(rowspan: 3)[台阶种类，分上半台阶、双层台阶和下半台阶], table.cell(fill: black)[`bottom`], [`double`], [`top`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[小型垂滴叶], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`half`], table.cell(rowspan: 2)[小型垂滴叶的上下半部分], table.cell(fill: black)[`lower`], [`upper`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  [嗅探者蛋], [`hatch`], [蛋的孵化阶段，默认为 `0`], [`0` \~ `2`],
  [雪], [`layer`], [雪的厚度，默认为1], [`1` \~ `8`],
  table.cell(rowspan: 13)[所有种类楼梯], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`half`], table.cell(rowspan: 2)[楼梯的正置或倒置], table.cell(fill: black)[`bottom`], [`top`],
  table.cell(rowspan: 5)[`shape`], table.cell(rowspan: 5)[楼梯形状分为左右内角楼梯、左右外角楼梯和普通直楼梯], [`inner_left`], [`inner_right`], [`outer_left`], [`outer_right`], table.cell(fill: black)[`straight`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[结构方块], table.cell(rowspan: 4)[`mode`], table.cell(rowspan: 4)[结构方块的模式], [`corner`], [`data`], table.cell(fill: black)[`load`], [`save`],
  [可疑的沙子、可疑的沙砾], [`dusted`], [可疑的方块被清刷的程度，默认为 `0`], [`0` \~ `3`],
  [甜浆果丛], [`age`], [甜浆果丛的生长程度，默认为 `0`], [`0` \~ `3`],
  table.cell(rowspan: 4)[测试方块], table.cell(rowspan: 4)[`mode`], table.cell(rowspan: 4)[测试方块的模式], [`accept`], [`fail`], [`log`], table.cell(fill: black)[`start`],
  table.cell(rowspan: 2)[TNT], table.cell(rowspan: 2)[`unstable`], table.cell(rowspan: 2)[TNT是否除了可以被打火石等点燃外，生存模式的玩家也可以破坏TNT以点燃TNT。若为否，则只能被打火石等点燃], table.cell(fill: black)[`false`], [`true`],
  [火把花植株], [`age`], [火把花植株的生长程度，默认为 `0`], [`0` \~ `1`],
  table.cell(rowspan: 12)[所有种类活板门], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`half`], table.cell(rowspan: 2)[活板门为上活板门还是下活板门], table.cell(fill: black)[`bottom`], [`top`],
  table.cell(rowspan: 2)[`open`], table.cell(rowspan: 2)[门的开关], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[试炼刷怪笼、不详试炼刷怪笼], table.cell(rowspan: 2)[`ominous`], table.cell(rowspan: 2)[试炼刷怪笼是否为不详变种], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[`trial_spawner_state`], table.cell(rowspan: 6)[试炼刷怪笼的激活状态：`inactive` 表示试炼刷怪笼处于未激活状态。`waiting_for_players` 表示试炼刷怪笼可以被激活，正在等待玩家加入试炼。`active` 表示试炼刷怪笼处于激活状态，可以刷新生物。`waiting_for_reward_ejection` 表示所有试炼生物已经被清除，等待喷出奖励。`ejecting_reward` 表示试炼刷怪笼喷出奖励。`cooldown` 表示试炼刷怪笼处于冷却阶段，无法被激活。], [`active`], [`cooldown`], [`ejecting_reward`], [`inactive`], [`waiting_for_players`], [`waiting_for_reward_ejection`],
  table.cell(rowspan: 14)[绊线], table.cell(rowspan: 2)[`attached`], table.cell(rowspan: 2)[绊线是否和有效的绊线钩连接在一起], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`disarmed`], table.cell(rowspan: 2)[绊线是否已经被拆除], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[绊线是否与东方的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[绊线是否与北方的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[绊线是否与南方的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[绊线是否与西方的方块连接], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 8)[绊线钩], table.cell(rowspan: 2)[`attached`], table.cell(rowspan: 2)[绊线钩是否和有效的绊线连接在一起], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[海龟蛋], [`eggs`], [海龟蛋的个数，默认为 `1`], [`1` \~ `4`],
  [`hatch`], [蛋的孵化阶段，默认为 `0`], [`0` \~ `2`],
  table.cell(rowspan: 10)[宝库、不详宝库], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`ominous`], table.cell(rowspan: 2)[试炼刷怪笼是否为不详变种], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 4)[`vault_state`], table.cell(rowspan: 4)[宝库的状态：`inactive` 表示宝库周围没有有效的玩家，处于未激活状态。`active` 表示宝库周围有有效的玩家，处于激活状态。`unlocking` 表示宝库被试炼钥匙解锁，在20 gt后进入喷出物品状态。`ejecting` 表示宝库喷出物品，物品全部喷出后回到激活或未激活状态], [`active`], [`ejecting`], [`inactive`], [`unlocking`],
  table.cell(rowspan: 10)[藤蔓], table.cell(rowspan: 2)[`east`], table.cell(rowspan: 2)[是否附着在其他方块的东方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`north`], table.cell(rowspan: 2)[是否附着在其他方块的北方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`south`], table.cell(rowspan: 2)[是否附着在其他方块的南方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`up`], table.cell(rowspan: 2)[是否附着在其他方块的上方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 2)[`west`], table.cell(rowspan: 2)[是否附着在其他方块的西方], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 16)[所有种类墙], table.cell(rowspan: 3)[`east`], table.cell(rowspan: 3)[墙是否与东面的方块连接、连接的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 3)[`north`], table.cell(rowspan: 3)[墙是否与北面的方块连接、连接的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 3)[`south`], table.cell(rowspan: 3)[墙是否与南面的方块连接、连接的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 2)[`up`], table.cell(rowspan: 2)[墙是否有中心柱], [`false`], table.cell(fill: black)[`true`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 3)[`west`], table.cell(rowspan: 3)[墙是否与西面的方块连接、连接的形状高低与否], table.cell(fill: black)[`none`], [`low`], [`tall`],
  table.cell(rowspan: 6)[所有种类墙上的生物头颅], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否被激活], table.cell(fill: black)[`false`], [`true`],
  table.cell(rowspan: 6)[所有种类墙上的告示牌、墙上的悬挂式告示牌、大型垂滴叶茎、末影箱、梯子], table.cell(rowspan: 4)[`facing`], table.cell(rowspan: 4)[方块的朝向], [`east`], table.cell(fill: black)[`north`], [`south`], [`west`],
  table.cell(rowspan: 2)[`waterlogged`], table.cell(rowspan: 2)[是否含水], table.cell(fill: black)[`false`], [`true`],
  [装有水的炼药锅、装有细雪的炼药锅], [`level`], [炼药锅内容物高度], [`1` \~ `3`],
  table.cell(rowspan: 2)[所有种类木质与石质压力板], table.cell(rowspan: 2)[`powered`], table.cell(rowspan: 2)[是否提供红石信号], table.cell(fill: black)[`false`], [`true`]
)
#pagebreak()
